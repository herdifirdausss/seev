...
// every type in services/ledger/internal/transport/http.go's publicUserTypes when
...
// receives a PolicyChecker at all — services/ledger/internal/transport/http.go's
...
// services/ledger/policy's Engine anchors "today"/"this month" to
// (services/ledger/internal/ledger/ledger.go's own LoadLocation call); a UTC-fallback
...
func ReconstructPolicyCounters(ctx context.Context, ledgerDB *sql.DB, counter cache.Counter, loc *time.Location, report *Report) error {
	...
	for txType, mapping := range reachablePolicyTypes {
		...
		if err := reconstructWindow(ctx, ledgerDB, counter, txType, accountColumn, dayStart, dayEnd,
			func(userID uuid.UUID, currency string) string {
				return policyKeyDailyAmount(userID, txType, currency, now)
			},
			func(userID uuid.UUID, currency string) string {
				return policyKeyDailyCount(userID, txType, currency, now)
			},
		...
		if err := reconstructWindow(ctx, ledgerDB, counter, txType, accountColumn, monthStart, monthEnd,
			func(userID uuid.UUID, currency string) string {
				return policyKeyMonthlyAmount(userID, txType, currency, now)
			},
