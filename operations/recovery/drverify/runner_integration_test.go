...
import (
	...
	"github.com/herdifirdausss/seev/operations/recovery/drverify"
)
...
func setupCluster(t *testing.T) map[string]string {
	...
	for _, service := range drverify.Services {
		...
		if err := testutil.ApplyMigration("file://../../../", service, dsn); err != nil {
...
// rejects a write — the same mechanism operations/recovery/drverify/db.go's
