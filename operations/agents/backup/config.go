// Package backupagent implements docs/roadmap/archive/50 T2's operational backup
...
// extension of internal/platform/config's shared Config/load() — backup-agent has
...
// databases (docs/roadmap/archive/50 T0 Result) — matches
...
type Config struct {
	// AppPort is the internal mTLS listener port — 8097 (docs/roadmap/archive/50 T0
	...
	// seev_backup role (docs/roadmap/archive/50 K5), never the schema owner.
	PostgresHost     string
	PostgresPort     string
	BackupDBUser     string
	...
	// lab-scale database — docs/roadmap/archive/50 §8 explicitly disclaims any
	...
	// time (docs/roadmap/archive/50 T2 Result).
	FullCronSpec string
	DiffCronSpec string
}
...
// resolution — found live in docs/roadmap/archive/50 T2 execution (pgbackrest tried
