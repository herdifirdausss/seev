...
func TestRing_OldKeyReadNewKeyWrite(t *testing.T) {
	keyV1 := randomKey(t)
	ringV1, err := NewRing(map[int][]byte{1: keyV1}, 1)
	require.NoError(t, err)

	oldEnvelope, err := ringV1.Seal(testAAD(), []byte("sealed under v1"))
	require.NoError(t, err)

	keyV2 := randomKey(t)
	ringV2, err := NewRing(map[int][]byte{1: keyV1, 2: keyV2}, 2)
	require.NoError(t, err)

	newEnvelope, err := ringV2.Seal(testAAD(), []byte("sealed under v2"))
	require.NoError(t, err)
	...
	gotOld, err := ringV2.Open(testAAD(), oldEnvelope)
	...
	gotNew, err := ringV2.Open(testAAD(), newEnvelope)
	require.NoError(t, err)
...
func TestRing_OpenUnknownKeyVersion_FailsClosed(t *testing.T) {
	...
	ringV1, err := NewRing(map[int][]byte{1: keyV1}, 1)
	require.NoError(t, err)
	envelope, err := ringV1.Seal(testAAD(), []byte("secret"))
	require.NoError(t, err)
	...
	ringWithoutV1, err := NewRing(map[int][]byte{2: randomKey(t)}, 2)
	require.NoError(t, err)
	_, err = ringWithoutV1.Open(testAAD(), envelope)
