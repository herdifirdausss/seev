...
// applyLedgerMigrations runs the real migrations/ledger/*.up.sql files
// against dsn — inlined rather than internal/testutil.ApplyMigration
// (a boundary_test.go rule: cmd/loaddataset, a standalone read-only tool
// with no owned internal/ module, may not import internal/testutil; the
...
func applyLedgerMigrations(t *testing.T, dsn string) {
	t.Helper()
	m, err := migrate.New("file://../../migrations/ledger", dsn+"&x-migrations-table=schema_migrations_ledger")
...
// reports on — mirrors internal/ledger/schema_contract_test.go's own
// createUserCashAccount pattern, kept local since this tool has no
// dependency on internal/ledger's repository package.
...
func TestCollect_ContentHashDeterministic(t *testing.T) {
	...
	m1 := collect(context.Background(), db, "", "run-a")
	m2 := collect(context.Background(), db, "", "run-b")
	require.Equal(t, m1.ContentHash, m2.ContentHash, "identical dataset shape must hash identically regardless of run_id")
}
