...
-- Prefixed with the owning service (adminbff_) rather than the unprefixed
...
CREATE TABLE adminbff_retention_holds (
    id            UUID PRIMARY KEY,
    ...
    CONSTRAINT chk_adminbff_retention_hold_release_fields CHECK (
    ...
    CONSTRAINT chk_adminbff_retention_hold_releaser_not_creator CHECK (
        released_by IS NULL OR released_by <> created_by
    )
);

CREATE INDEX idx_adminbff_retention_holds_scope  ON adminbff_retention_holds(scope, scope_value) WHERE status = 'active';
CREATE INDEX idx_adminbff_retention_holds_expiry ON adminbff_retention_holds(expires_at) WHERE status = 'active' AND expires_at IS NOT NULL;
...
CREATE TABLE adminbff_retention_audit (
...
CREATE INDEX idx_adminbff_retention_audit_class ON adminbff_retention_audit(class, finished_at DESC);
CREATE INDEX idx_adminbff_retention_audit_job   ON adminbff_retention_audit(job_id);

GRANT SELECT, INSERT, UPDATE ON adminbff_retention_holds TO app_service;
GRANT SELECT, INSERT ON adminbff_retention_audit TO app_service;
GRANT SELECT ON adminbff_retention_holds, adminbff_retention_audit TO app_readonly;

ALTER TABLE adminbff_retention_holds ENABLE ROW LEVEL SECURITY;
ALTER TABLE adminbff_retention_audit ENABLE ROW LEVEL SECURITY;
ALTER TABLE adminbff_retention_holds FORCE ROW LEVEL SECURITY;
ALTER TABLE adminbff_retention_audit FORCE ROW LEVEL SECURITY;

CREATE POLICY adminbff_retention_holds_service  ON adminbff_retention_holds FOR ALL    TO app_service  USING (true) WITH CHECK (true);
CREATE POLICY adminbff_retention_holds_readonly ON adminbff_retention_holds FOR SELECT TO app_readonly USING (true);
CREATE POLICY adminbff_retention_audit_service  ON adminbff_retention_audit FOR ALL    TO app_service  USING (true) WITH CHECK (true);
CREATE POLICY adminbff_retention_audit_readonly ON adminbff_retention_audit FOR SELECT TO app_readonly USING (true);
...
CREATE OR REPLACE FUNCTION fn_adminbff_retention_hold_covers(
    p_table    TEXT,
    p_subject  TEXT,
    p_resource TEXT,
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = pg_catalog, public AS $$
    SELECT EXISTS (
        SELECT 1 FROM adminbff_retention_holds h
...
GRANT EXECUTE ON FUNCTION fn_adminbff_retention_hold_covers(TEXT, TEXT, TEXT, TIMESTAMPTZ) TO app_service;
