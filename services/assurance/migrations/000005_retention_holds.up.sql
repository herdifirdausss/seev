...
-- Prefixed with the owning service (assurance_) rather than the unprefixed
-- "retention_holds" every service would otherwise share by coincidence:
-- several of this repo's own integration test suites (internal/testkit's
...
CREATE TABLE assurance_retention_holds (
    id            UUID PRIMARY KEY,
    ...
    CONSTRAINT chk_assurance_retention_hold_release_fields CHECK (
    ...
    CONSTRAINT chk_assurance_retention_hold_releaser_not_creator CHECK (
        released_by IS NULL OR released_by <> created_by
    )
);

CREATE INDEX idx_assurance_retention_holds_scope  ON assurance_retention_holds(scope, scope_value) WHERE status = 'active';
CREATE INDEX idx_assurance_retention_holds_expiry ON assurance_retention_holds(expires_at) WHERE status = 'active' AND expires_at IS NOT NULL;
...
CREATE TABLE assurance_retention_audit (
...
CREATE INDEX idx_assurance_retention_audit_class ON assurance_retention_audit(class, finished_at DESC);
CREATE INDEX idx_assurance_retention_audit_job   ON assurance_retention_audit(job_id);

GRANT SELECT, INSERT, UPDATE ON assurance_retention_holds TO app_service;
GRANT SELECT, INSERT ON assurance_retention_audit TO app_service;
GRANT SELECT ON assurance_retention_holds, assurance_retention_audit TO app_readonly;

ALTER TABLE assurance_retention_holds ENABLE ROW LEVEL SECURITY;
ALTER TABLE assurance_retention_audit ENABLE ROW LEVEL SECURITY;
ALTER TABLE assurance_retention_holds FORCE ROW LEVEL SECURITY;
ALTER TABLE assurance_retention_audit FORCE ROW LEVEL SECURITY;

CREATE POLICY assurance_retention_holds_service  ON assurance_retention_holds FOR ALL    TO app_service  USING (true) WITH CHECK (true);
CREATE POLICY assurance_retention_holds_readonly ON assurance_retention_holds FOR SELECT TO app_readonly USING (true);
CREATE POLICY assurance_retention_audit_service  ON assurance_retention_audit FOR ALL    TO app_service  USING (true) WITH CHECK (true);
CREATE POLICY assurance_retention_audit_readonly ON assurance_retention_audit FOR SELECT TO app_readonly USING (true);
...
CREATE OR REPLACE FUNCTION fn_assurance_retention_hold_covers(
    p_table    TEXT,
    p_subject  TEXT,
    p_resource TEXT,
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = pg_catalog, public AS $$
    SELECT EXISTS (
        SELECT 1 FROM assurance_retention_holds h
...
GRANT EXECUTE ON FUNCTION fn_assurance_retention_hold_covers(TEXT, TEXT, TEXT, TIMESTAMPTZ) TO app_service;
