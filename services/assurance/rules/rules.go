...
// for exactly the resolved gateway value (both services/assurance's own
// gRPC lookup and operations/recovery/drverify/cmd/drverify's direct-SQL one), so every proof here is
