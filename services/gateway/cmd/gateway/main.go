...
import (
	...
	"github.com/herdifirdausss/seev/internal/platform/transport/httpserver"
	"github.com/herdifirdausss/seev/services/gateway/internal/merchant"
	merchantapi "github.com/herdifirdausss/seev/services/gateway/internal/merchant/api"
	merchantclient "github.com/herdifirdausss/seev/services/gateway/internal/merchant/client"
	"github.com/herdifirdausss/seev/services/gateway/internal/merchant/idempotency"
	"github.com/herdifirdausss/seev/services/gateway/internal/merchant/quota"
	"github.com/herdifirdausss/seev/services/gateway/internal/notification/channel"
	"github.com/herdifirdausss/seev/services/gateway/internal/notification/channel/email"
	"github.com/herdifirdausss/seev/services/gateway/internal/notification/channel/push"
	transporthttp "github.com/herdifirdausss/seev/services/gateway/internal/transport/http"
...
func main() {
	...
	// of services/gateway/internal/merchant.Module into services/gateway/cmd/gateway — T2 through T7 built
	...
	// seev_merchant_webhook_*) — see services/gateway/internal/merchant/orchestration/metrics.go.
	...
	// (transporthttp.NewRouter's rate limiter, Ready's health check) must
	// nil-check it rather than assume it's always populated.
	deps := &transporthttp.Dependencies{
		DB:          db,
		Cache:       transporthttp.CacheOrNil(redisCache),
		MQ:          mq,
	...
	publicRouter := transporthttp.NewRouter(cfg, deps, log)
	internalRouter := transporthttp.NewInternalRouter(cfg, deps, log)
