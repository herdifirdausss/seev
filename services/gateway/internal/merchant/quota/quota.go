// Package quota is services/gateway/internal/merchant's per-tenant rate-limit enforcement
// (docs/roadmap/archive/57-c1-merchant-b2b-api.md §3.1, T4). It reuses
// internal/platform/cache's existing atomic Redis token-bucket Lua script
// (internal/platform/cache.RedisRateLimiter) rather than writing a second one — the only
// thing this package adds is PER-TENANT dynamic rate/burst (the shared
// internal/platform/cache type bakes its RateConfig in at construction, which doesn't fit
