...
func TestEnforcer_Check_AllowsWithinBurstThenRejects(t *testing.T) {
	...
	firstCheck, err := enforcer.Check(context.Background(), tenantID, "transfers", true)
	require.NoError(t, err)
	assert.True(t, firstCheck.Allowed)

	secondCheck, err := enforcer.Check(context.Background(), tenantID, "transfers", true)
	require.NoError(t, err)
	assert.True(t, secondCheck.Allowed)

	burstExceededCheck, err := enforcer.Check(context.Background(), tenantID, "transfers", true)
	require.NoError(t, err)
	assert.False(t, burstExceededCheck.Allowed, "the third request must exceed a burst of 2")
	assert.Positive(t, burstExceededCheck.ResetSeconds)
}
...
func TestEnforcer_Check_TenantsAreIndependent(t *testing.T) {
	...
	tenantACheck, err := enforcer.Check(context.Background(), tenantA, "transfers", true)
	require.NoError(t, err)
	assert.True(t, tenantACheck.Allowed)

	// Exhaust tenant A's burst.
	tenantAExceededCheck, err := enforcer.Check(context.Background(), tenantA, "transfers", true)
	require.NoError(t, err)
	assert.False(t, tenantAExceededCheck.Allowed)

	// Tenant B is unaffected.
	tenantBCheck, err := enforcer.Check(context.Background(), tenantB, "transfers", true)
	require.NoError(t, err)
	assert.True(t, tenantBCheck.Allowed, "one tenant exhausting its quota must never affect another tenant")
}
