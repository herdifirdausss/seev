// Package api holds services/gateway/internal/merchant's HTTP handlers and DTO mapping
...
// (contracts/http/b2b-v1.yaml's own locked decision — see that file's
// `responses:` section comment) via internal/platform/transport/http/response; the B2B-specific
// addition is exact error codes from §6.7's stable list, which
// internal/platform/transport/http/response's own named helpers (NotFound, BadRequest, ...) do not
...
// ownerErrorStatus maps services/gateway/internal/merchant/client's three-sentinel error
