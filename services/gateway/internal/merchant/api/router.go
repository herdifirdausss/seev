...
// surface — deliberately narrower than the full services/gateway/internal/merchant.Module
...
type Deps struct {
	...
	// (services/gateway/internal/merchant/auth.GlobalFlag) — T9's own Result section
...
// convention services/gateway/internal/transport/http.NewRouter already uses for its own sub-mux
...
// create handler calls directly (services/gateway/internal/merchant/idempotency.Service has
...
func NewRouter(deps Deps) http.Handler {
	...
	// T9's kill switch runs BEFORE auth (services/gateway/internal/merchant/auth.GlobalFlag's
