...
// defaultQuotaBaselineRPM/Burst mirror services/gateway/internal/merchant/quota's own
...
// services/gateway/cmd/gateway behind the internal listener's JWT `authed` chain (the same
...
// "/api/v1/admin/gateway/") in services/adminbff/internal/admin/module.go) — CSRF and
...
// services/ledger/internal/transport/http.go's own isAdmin/isAdminMaker/
// isAdminChecker (this codebase duplicates this trio per package rather
// than sharing one, an established precedent — see
// services/auth/internal/transport/http/operator_offboarding_http.go's own isAdminMaker/
...
// the JWT subject, matching services/auth's own "operator identity (email
...
func (m *Module) adminCreateTenant(w http.ResponseWriter, r *http.Request) {
	...
	tenant := model.Tenant{
		ID: identifiers.NewV7(), PublicID: "tn_" + uuid.NewString()[:16], ExternalCode: req.ExternalCode,
...
// tenant's own suspension. See services/gateway/internal/merchant/auth.GlobalFlag's own
