...
// for services/gateway/internal/merchant subpackages) ──────────────────────────────────────
...
// fakeLifecycleRepo mirrors services/gateway/internal/merchant/lifecycle's own test fake —
...
// services/gateway/internal/merchant/webhook package already has full coverage of the
...
// services/gateway/cmd/gateway's own internal router uses (services/gateway/internal/transport/http/router.go) —
// matches services/ledger/internal/transport's own established test pattern of
...
func TestAdminRouter_SuspendTenant_MakerOnly(t *testing.T) {
	...
	suspendResponse := doRequest(t, router, http.MethodPost, "/admin/gateway/tenants/"+tenantID.String()+"/suspend", "admin_maker", nil)
	assert.Equal(t, http.StatusNoContent, suspendResponse.Code)
...
func TestAdminRouter_UpdateQuota_AboveBaselineRequiresChecker(t *testing.T) {
	...
	checkerResponse := doRequest(t, router, http.MethodPut, "/admin/gateway/tenants/"+tenantID.String()+"/quota", "admin_checker", map[string]any{
		"requests_per_minute": 500, "burst": 500, "is_enabled": true,
	})
	assert.Equal(t, http.StatusOK, checkerResponse.Code, checkerResponse.Body.String())
}
...
// auth middleware chain (matching services/gateway/cmd/gateway's own wiring): create a
...
// contracts/clients/ledger's own tests and this file's UnavailableWithoutLedgerClient
// case above.
...
func TestAdminRouter_GlobalFlag_CheckerCanDisableAndReenable(t *testing.T) {
	...
	// RequireB2BEnabled itself (services/gateway/internal/merchant/auth) is proven
