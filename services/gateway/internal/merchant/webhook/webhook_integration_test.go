...
// setupGatewayTestDB mirrors services/gateway/internal/merchant/repository's own
...
func merchantEventDelivery(t *testing.T, tenantID uuid.UUID) amqp.Delivery {
	...
	ev := events.NewTransactionPosted(
		identifiers.NewV7(), "merchant_transfer", "50000", "IDR",
...
func TestWebhookRelay_EndToEnd(t *testing.T) {
	...
	processedReplayCount, _, err := relay.ProcessOnce(ctx)
	require.NoError(t, err)
	require.Equal(t, 1, processedReplayCount, "the replay delivery must be picked up and dispatched on the next poll")
	...
	staleDelivery := model.WebhookDelivery{
		ID: identifiers.NewV7(), PublicID: "whd_stale", TenantID: tenantID,
	...
	processedExpiredLeaseCount, _, err := relay.ProcessOnce(ctx)
	require.NoError(t, err)
	require.Equal(t, 1, processedExpiredLeaseCount, "ClaimDue must reclaim a delivery whose lease already expired")
...
func capturedConsumerHandler(t *testing.T, webhooks repository.WebhookRepository, tenants repository.TenantRepository) messaging.HandlerFunc {
	...
	// services/gateway/internal/notification.Module.Start's own async shape) — capturedCh
