...
func TestService_Replay_NewDeliveryIDSameEventID(t *testing.T) {
	...
	original := model.WebhookDelivery{
		ID: identifiers.NewV7(), PublicID: "whd_orig", TenantID: tenantID,
...
func TestService_Replay_ExemptFromAutomaticDedup(t *testing.T) {
	...
	firstReplay, err := svc.Replay(context.Background(), tenantID, original.ID)
	require.NoError(t, err)
	secondReplay, err := svc.Replay(context.Background(), tenantID, original.ID)
	require.NoError(t, err)

	assert.NotEqual(t, firstReplay.ID, secondReplay.ID)
...
func TestService_Replay_WrongTenantNotFound(t *testing.T) {
	...
	svc := NewService(repo, ring)
	tenantID := identifiers.NewV7()
	otherTenantID := identifiers.NewV7()
