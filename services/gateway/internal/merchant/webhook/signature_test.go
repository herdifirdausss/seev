...
func FuzzParseSignatureHeader(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, header string) {
		...
		timestamp, signatureV1, ok := parseSignatureHeader(header)
		if ok && (timestamp == 0 || signatureV1 == "") {
			t.Fatalf("parseSignatureHeader(%q) returned ok=true with an invalid zero-value result (t=%d, v1=%q)", header, timestamp, signatureV1)
		}
	})
}
