...
const (
	...
	// services/ledger/internal/repository/outbox_event_repository.go's MarkFailed):
...
// interval — matches internal/platform/lifecycle/objectoutbox.Worker.Start's own Start/Stop
...
func (w *RelayWorker) processDelivery(ctx context.Context, d model.WebhookDelivery) error {
	...
	attempt := model.WebhookAttempt{
		ID: identifiers.NewV7(), DeliveryID: d.ID, AttemptNumber: attemptNumber,
...
// services/payout/internal/repository/vendor_command_repository.go's FailCommand.
