...
func Verify(secret []byte, header string, body []byte) bool {
	signedAtUnix, signatureV1, ok := parseSignatureHeader(header)
	if !ok {
		return false
	}
	want := Sign(secret, signedAtUnix, body)
	_, expectedSignatureV1, _ := parseSignatureHeader(want)
	return hmac.Equal([]byte(signatureV1), []byte(expectedSignatureV1))
}
...
func VerifyWithTolerance(secret []byte, header string, body []byte, now time.Time, tolerance time.Duration) bool {
	signedAtUnix, _, ok := parseSignatureHeader(header)
	if !ok {
		return false
	}
...
func parseSignatureHeader(header string) (timestamp int64, signatureV1 string, ok bool) {
	...
	if timestamp == 0 || signatureV1 == "" {
		return 0, "", false
	}
	return timestamp, signatureV1, true
}
