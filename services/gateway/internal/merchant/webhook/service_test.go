...
func TestService_CreateEndpoint_RejectsInvalidEnvironment(t *testing.T) {
	...
	_, _, err := svc.CreateEndpoint(context.Background(), identifiers.NewV7(), "https://example.test/hook", "production", []string{transactionPostedExternalType}, nil)
	assert.ErrorIs(t, err, ErrInvalidEnvironment)
}

func TestService_CreateEndpoint_RejectsEmptySubscriptions(t *testing.T) {
	svc := NewService(newFakeWebhookRepository(), testRing(t))
	_, _, err := svc.CreateEndpoint(context.Background(), identifiers.NewV7(), "https://example.test/hook", "live", nil, nil)
...
func TestService_CreateEndpoint_RejectsEmptyURL(t *testing.T) {
	svc := NewService(newFakeWebhookRepository(), testRing(t))
	_, _, err := svc.CreateEndpoint(context.Background(), identifiers.NewV7(), "", "live", []string{transactionPostedExternalType}, nil)
...
func TestService_CreateEndpoint_RejectsInvalidURL(t *testing.T) {
	...
	for _, raw := range cases {
		svc := NewService(newFakeWebhookRepository(), testRing(t))
		_, _, err := svc.CreateEndpoint(context.Background(), identifiers.NewV7(), raw, "live", []string{transactionPostedExternalType}, nil)
