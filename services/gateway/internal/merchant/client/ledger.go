...
// MerchantTransfer processor registers under (services/ledger/internal/processors/merchant_transfer.go).
...
// own type, not contracts/clients/ledger.Transaction, so services/gateway/internal/merchant/api's
...
// — satisfied by both *contracts/clients/ledger.Client (the real gRPC-connected
// client, used in production) and *internal/testkit.LedgerHarness (the
