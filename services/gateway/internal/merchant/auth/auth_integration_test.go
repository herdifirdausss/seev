...
// setupGatewayTestDB mirrors services/gateway/internal/merchant/repository's own helper —
...
func TestKeyServiceAndMiddleware_RealStack(t *testing.T) {
	...
	deniedRequest := httptest.NewRequest(http.MethodPost, "/api/v1/b2b/transfers", nil)
	deniedRequest.Header.Set("Authorization", "Bearer "+plaintext)
	...
	handler.ServeHTTP(revokedResponse, req)
	require.Equal(t, http.StatusUnauthorized, revokedResponse.Code, "revocation must take effect immediately against the real database")
}
