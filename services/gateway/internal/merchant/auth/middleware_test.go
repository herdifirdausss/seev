...
func TestRequireScope_DeniesMissingScopeAndAllowsGranted(t *testing.T) {
	...
	insufficientPrincipal := Principal{TenantID: uuid.New(), Scopes: []string{"transfers:write"}} // missing transactions:read
	deniedRequest := httptest.NewRequest(http.MethodGet, "/", nil)
	deniedRequest = deniedRequest.WithContext(WithPrincipal(deniedRequest.Context(), insufficientPrincipal))
	secondHandler := RequireScope("b2bCreateTransferV1")(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		t.Fatal("handler must not run when scope is insufficient")
	}))
	secondResponse := httptest.NewRecorder()
	secondHandler.ServeHTTP(secondResponse, deniedRequest)
