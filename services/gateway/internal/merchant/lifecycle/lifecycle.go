...
// mirrors services/auth's own operator-offboarding propose/approve/reject
...
// convention, matching services/auth.ProposeOperatorOffboarding's
...
func (s *Service) Propose(ctx context.Context, tenantID uuid.UUID, action, requestedBy, reason string) (model.TenantLifecycleRequest, error) {
	...
	req := model.TenantLifecycleRequest{
		ID: identifiers.NewV7(), TenantID: tenantID, Action: action, RequestedBy: requestedBy, Reason: reason,
	}
...
// services/auth.ApproveOperatorOffboarding). On success, applies the
