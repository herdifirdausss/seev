...
// newTenant mirrors services/gateway/internal/merchant/repository's own test helper —
...
// services/gateway/internal/merchant/auth's own helper — ledger migrations run first
...
func TestService_Begin_CrashRecovery_RealPostgres(t *testing.T) {
	...
	recoveryService := idempotency.NewService(repo, time.Hour, "process-b")
	recovered, err := recoveryService.Begin(context.Background(), tenantID, "b2bCreateTransferV1", "crash-key", body)
	require.NoError(t, err)
	...
	require.NoError(t, recoveryService.Complete(context.Background(), tenantID, recovered.RecordID, 201, []byte(`{"id":"tx_recovered"}`), nil, nil))

	replay, err := recoveryService.Begin(context.Background(), tenantID, "b2bCreateTransferV1", "crash-key", body)
	require.NoError(t, err)
