...
// real handlers behind real WithAuth (e.g. services/payout/internal/transport/http/http_test.go
// TestCreateHandler_NoToken_401, services/auth integration tests).
...
func TestRequireKYCForLedgerPostings_GatesOnlyPostTransactions(t *testing.T) {
	...
	levelZeroToken := tokenWithKYCLevel(t, 0)
	levelOneToken := tokenWithKYCLevel(t, 1)

	w := doRequest(t, router, http.MethodPost, "/api/v1/ledger/transactions",
		map[string]string{"Authorization": "Bearer " + levelZeroToken, "Content-Type": "application/json"})
	...
	w = doRequest(t, router, http.MethodPost, "/api/v1/ledger/transactions",
		map[string]string{"Authorization": "Bearer " + levelOneToken, "Content-Type": "application/json"})
	...
	w = doRequest(t, router, http.MethodGet, "/api/v1/ledger/accounts",
		map[string]string{"Authorization": "Bearer " + levelZeroToken})
...
func TestRequireKYC_ExactRouteMount_GatesUnconditionally(t *testing.T) {
	...
	for _, path := range []string{"/payout", "/topup"} {
		...
		req.Header.Set("Authorization", "Bearer "+levelZeroToken)
		w := httptest.NewRecorder()
		...
		secondRequest.Header.Set("Authorization", "Bearer "+levelOneToken)
		secondResponse := httptest.NewRecorder()
		chain.ServeHTTP(secondResponse, secondRequest)
		assert.Equal(t, http.StatusOK, secondResponse.Code, "L1 at exact-match route %s must pass", path)
	}
}
