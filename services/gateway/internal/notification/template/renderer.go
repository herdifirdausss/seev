...
func Builtin(kind, channel, locale string) (Version, bool) {
	...
	case model.KindDisputeLifecycle:
		v.TitleTemplate = "Dispute update"
		v.BodyTextTemplate = "Dispute {{.Dispute.Reference}} is now {{.Dispute.Status}} for " + amount + ".{{if .Dispute.EvidenceDueAt}} Evidence is due by {{.Dispute.EvidenceDueAt}}.{{end}}"
		v.SubjectTemplate = "Your dispute has been updated"
	default:
		return Version{}, false
	}
	if locale == "id-ID" {
		...
		case model.KindDisputeLifecycle:
			v.TitleTemplate = "Pembaruan sengketa"
			v.BodyTextTemplate = "Sengketa {{.Dispute.Reference}} Anda sekarang berstatus {{.Dispute.Status}} untuk " + amount + ".{{if .Dispute.EvidenceDueAt}} Bukti harus dikirim sebelum {{.Dispute.EvidenceDueAt}}.{{end}}"
			v.SubjectTemplate = "Pembaruan sengketa Anda"
		}
	}
	switch channel {
...
func ValidateRenderContext(value model.RenderContext) error {
	...
	if value.Dispute.ID != "" {
		if _, err := uuid.Parse(value.Dispute.ID); err != nil {
			return errors.New("render context dispute id is invalid")
		}
		if value.Dispute.Reference == "" || value.Dispute.Status == "" {
			return errors.New("render context dispute fields are incomplete")
		}
		switch value.Dispute.Status {
		case "open", "evidence_submitted", "won", "lost", "expired":
		default:
			return errors.New("render context dispute status is invalid")
		}
		if value.Dispute.EvidenceDueAt != "" {
			if _, err := time.Parse(time.RFC3339, value.Dispute.EvidenceDueAt); err != nil {
				return errors.New("render context dispute deadline is invalid")
			}
		}
	}
