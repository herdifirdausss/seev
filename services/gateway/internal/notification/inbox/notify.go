...
// only import this package; services/gateway/internal/notification/repository and
// services/gateway/internal/notification/model are private to the module (docs/roadmap/archive/01 Module
// Boundaries, enforced by boundary_test.go). The ONLY services/ledger
// subpackage this module imports is contracts/events/ledger — the
...
// services/gateway/internal/notification/model.
...
// a local structural interface (mirrors services/payin's Poster pattern)
...
func (m *Module) Start(ctx context.Context) error {
	...
	if err := m.broker.DeclareTopology(ctx, messaging.QueueConfig{
		Queue:       queueName,
		RoutingKeys: []string{events.TypeTransactionPosted, events.TypeFXConversionPosted, events.TypeDisputeLifecycle},
	}); err != nil {
...
// redelivery-on-first-nack behavior (internal/platform/messaging's shouldRequeue: retry
...
func (m *Module) handleDelivery(ctx context.Context, d amqp.Delivery) error {
	if d.RoutingKey == events.TypeDisputeLifecycle {
		return m.handleDisputeLifecycleDelivery(ctx, d)
	}
...
func (m *Module) handleDisputeLifecycleDelivery(ctx context.Context, d amqp.Delivery) (err error) {
	started := time.Now()
	result := "error"
	defer func() {
		notificationEventsTotal.WithLabelValues("ledger", events.TypeDisputeLifecycle, result).Inc()
		notificationEventDuration.WithLabelValues("ledger", events.TypeDisputeLifecycle, result).Observe(time.Since(started).Seconds())
	}()

	var ev events.DisputeLifecycle
	if err := json.Unmarshal(d.Body, &ev); err != nil {
		return fmt.Errorf("notify: decode DisputeLifecycle: %w", err)
	}
	if err := ev.Validate(); err != nil {
		return fmt.Errorf("notify: validate DisputeLifecycle: %w", err)
	}
	if !m.config.Enabled || !m.config.InAppEnabled || ev.RecipientUserID == nil {
		result = "filtered"
		return nil
	}
	...
	eventID := *ev.EventID
	if m.platform != nil && m.db != nil {
		if err := m.handleModernDisputeDelivery(ctx, d, ev, eventID); err != nil {
			return err
		}
		result = "processed"
		return nil
	...
	kind, err := registry.KindForDispute()
	if err != nil {
		return err
	}
	notificationID := identifiers.NewV7()
	renderContext := disputeRenderContext(ev, notificationID)
	version, ok := notifytemplate.Builtin(kind.Kind, model.ChannelInApp, m.config.DefaultLocale)
	if !ok {
		return fmt.Errorf("notify: missing built-in in-app template for %s", kind.Kind)
	}
	rendered, err := m.renderer.Render(version, renderContext)
	if err != nil {
		return fmt.Errorf("notify: render dispute notification: %w", err)
	}
	_, err = m.repo.Insert(ctx, model.Notification{
		ID: notificationID, UserID: *ev.RecipientUserID, EventID: eventID,
		Type: events.TypeDisputeLifecycle, Title: rendered.Title, Body: rendered.Text, Payload: d.Body,
	})
	if err != nil {
		return fmt.Errorf("notify: insert dispute notification: %w", err)
	}
	result = "processed"
	return nil
}

func disputeRenderContext(ev events.DisputeLifecycle, notificationID uuid.UUID) model.RenderContext {
	dueAt := ""
	if ev.EvidenceDueAt != nil {
		dueAt = ev.EvidenceDueAt.UTC().Format(time.RFC3339)
	}
	context := model.RenderContext{
		NotificationID: notificationID.String(),
		Amount:         model.MoneyContext{Minor: ev.Amount, Currency: ev.Currency},
		Transaction:    model.TransactionContext{ID: ev.OriginalTxID.String(), PostedAt: ev.OccurredAt},
		Action:         model.ActionContext{DeepLink: "/notifications"},
		Dispute: model.DisputeContext{ID: ev.DisputeID.String(), Reference: ev.DisputeRef,
			Status: ev.ToStatus, EvidenceDueAt: dueAt},
	}
	context.Amount.Display = notifytemplate.FormatMoney(context.Amount)
	return context
}
...
func (m *Module) handleModernDelivery(ctx context.Context, d amqp.Delivery, ev events.TransactionPosted, eventID uuid.UUID) (err error) {
	...
	failureInbox := model.EventInbox{ID: identifiers.NewV7(), SourceService: "ledger", EventID: eventID,
	...
	for _, recipient := range modernRecipientsFor(ev) {
		...
		deliveries := []model.Delivery{{
			ID: identifiers.NewV7(), NotificationID: &notification.ID, UserID: notification.UserID,
		...
		inbox := model.EventInbox{ID: identifiers.NewV7(), SourceService: "ledger", EventID: eventID,
			EventType: events.TypeTransactionPosted, SchemaVersion: ev.SchemaVersion, PayloadHash: sha256Bytes(d.Body),
...
func (m *Module) handleModernDisputeDelivery(ctx context.Context, d amqp.Delivery, ev events.DisputeLifecycle, eventID uuid.UUID) (err error) {
	kind, err := registry.KindForDispute()
	if err != nil {
		return err
	}
	failureInbox := model.EventInbox{ID: identifiers.NewV7(), SourceService: "ledger", EventID: eventID,
		EventType: events.TypeDisputeLifecycle, SchemaVersion: ev.SchemaVersion, PayloadHash: sha256Bytes(d.Body),
		Status: "failed", ReceivedAt: time.Now().UTC()}
	defer func() {
		if err == nil {
			return
		}
		notificationPlanningFailuresTotal.WithLabelValues(kind.Kind, "planner_error").Inc()
		if recordErr := m.platform.RecordEventFailure(ctx, failureInbox, "planner_error"); recordErr != nil {
			m.logger.Warn("notify: record dispute planning failure failed", slog.Any("error", recordErr))
		}
	}()
	...
	contextJSON, err := json.Marshal(renderContext)
	if err != nil {
		return fmt.Errorf("notify: encode dispute render context: %w", err)
	}
	inAppVersion, ok, err := m.activeTemplate(ctx, kind.Kind, model.ChannelInApp, m.config.DefaultLocale)
	if err != nil {
		return fmt.Errorf("notify: load in-app dispute template: %w", err)
	}
	if !ok {
		notificationTemplateMissingTotal.WithLabelValues(model.ChannelInApp, kind.Kind, m.config.DefaultLocale).Inc()
		notificationPlanningFailuresTotal.WithLabelValues(kind.Kind, "in_app_template_missing").Inc()
	...
	if err != nil {
		notificationTemplateRenderTotal.WithLabelValues(model.ChannelInApp, kind.Kind, m.config.DefaultLocale, "error").Inc()
		return fmt.Errorf("notify: render in-app dispute notification: %w", err)
	}
	notificationTemplateRenderTotal.WithLabelValues(model.ChannelInApp, kind.Kind, m.config.DefaultLocale, "success").Inc()
	contentHash := sha256.Sum256(inApp.Payload)
	notification := model.Notification{
		ID: notificationID, UserID: *ev.RecipientUserID, EventID: eventID, Type: events.TypeDisputeLifecycle,
		Title: inApp.Title, Body: inApp.Text, Payload: []byte(`{}`), EventType: events.TypeDisputeLifecycle,
		SourceService: "ledger", Kind: kind.Kind, Category: kind.Category, Priority: kind.Priority,
		Requirement: kind.Requirement, Locale: m.config.DefaultLocale, TemplateVersionID: &inAppVersion.ID,
		DeepLink: kind.DeepLinkPath, Context: contextJSON, ContentHash: contentHash[:], CreatedAt: time.Now().UTC(),
	}
	deliveries := []model.Delivery{{
		ID: identifiers.NewV7(), NotificationID: &notification.ID, UserID: notification.UserID,
		Channel: model.ChannelInApp, Status: model.DeliveryDelivered, TemplateVersionID: inAppVersion.ID,
	...
	settings, err := m.platform.GetSettings(ctx, notification.UserID)
	if err != nil {
		return fmt.Errorf("notify: load dispute notification settings: %w", err)
	}
	settings = m.applySettingsDefaults(settings)
	preferences, err := m.platform.ListPreferences(ctx, notification.UserID)
	if err != nil {
		return fmt.Errorf("notify: load dispute notification preferences: %w", err)
	}
	digestItems := make([]model.DigestRequest, 0)
	for _, channelName := range []string{model.ChannelEmail, model.ChannelPush} {
		mode := effectiveMode(kind, channelName, preferences, m.config)
		if mode == model.ModeDisabled {
			continue
		}
		// Dispute lifecycle notices are deadline/compliance-sensitive and are
		// never moved into a digest, even if a stale preference requests it.
		if mode == model.ModeDailyDigest {
			continue
		}
		allowed, controlErr := m.channelAllowsPlanning(ctx, channelName)
		...
		version, ok, err := m.activeTemplate(ctx, kind.Kind, channelName, notification.Locale)
		if err != nil {
			return fmt.Errorf("notify: load %s dispute template: %w", channelName, err)
		}
		if !ok {
			notificationTemplateMissingTotal.WithLabelValues(channelName, kind.Kind, notification.Locale).Inc()
			notificationBlockedTotal.WithLabelValues(channelName, "template_missing").Inc()
			if channelName == model.ChannelPush {
				devices, deviceErr := m.platform.ListActiveDevices(ctx, notification.UserID)
				if deviceErr != nil {
					return fmt.Errorf("notify: list active devices for blocked dispute push: %w", deviceErr)
				}
				for _, device := range devices {
					endpointID := device.ID
					deliveries = append(deliveries, model.Delivery{
						ID: identifiers.NewV7(), NotificationID: &notification.ID, UserID: notification.UserID,
						Channel: channelName, EndpointID: &endpointID, EndpointIdentity: device.ID.String(), Status: model.DeliveryBlocked,
						TemplateVersionID: uuid.Nil, Locale: notification.Locale, ContentHash: sha256Bytes(nil), CreatedAt: time.Now().UTC(),
					})
					notificationDeliveriesTotal.WithLabelValues(channelName, kind.Kind, "blocked").Inc()
				}
			} else {
		...
		if err != nil {
			notificationTemplateRenderTotal.WithLabelValues(channelName, kind.Kind, notification.Locale, "error").Inc()
			return fmt.Errorf("notify: render %s dispute notification: %w", channelName, err)
		}
		notificationTemplateRenderTotal.WithLabelValues(channelName, kind.Kind, notification.Locale, "success").Inc()
		if channelName == model.ChannelEmail {
			deliveries = append(deliveries, model.Delivery{
		...
		devices, err := m.platform.ListActiveDevices(ctx, notification.UserID)
		if err != nil {
			return fmt.Errorf("notify: list active devices for dispute notification: %w", err)
		}
		for _, device := range devices {
			endpointID := device.ID
			deliveries = append(deliveries, model.Delivery{
	...
	inserted, err := m.platform.Plan(ctx, model.PlannedEvent{Inbox: inbox, Notification: notification, Deliveries: deliveries, DigestItems: digestItems})
	if err != nil {
		return fmt.Errorf("notify: plan dispute event: %w", err)
	}
	if inserted {
		notificationLogicalCreatedTotal.WithLabelValues(kind.Kind, kind.Category).Inc()
	} else {
		notificationDuplicatesTotal.WithLabelValues("ledger", events.TypeDisputeLifecycle).Inc()
	}
	return nil
}
