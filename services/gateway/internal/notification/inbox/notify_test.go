...
func TestHandleDelivery_DisputeLifecycle_InsertsComplianceNotification(t *testing.T) {
	m, repo := newModule(t)
	m.config = DefaultConfig()
	m.renderer = notifytemplate.NewRenderer(0, 0)
	disputeID, originalTxID, userID := uuid.New(), uuid.New(), uuid.New()
	due := time.Now().UTC().Add(24 * time.Hour)
	ev := events.NewDisputeLifecycle(disputeID, originalTxID, &userID, "dp-1", "visa", "10.4", "500000", "IDR", "", "open", &due, time.Now().UTC())
	body, err := json.Marshal(ev)
	require.NoError(t, err)
	repo.EXPECT().Insert(gomock.Any(), gomock.Any()).DoAndReturn(
		func(_ context.Context, n model.Notification) (bool, error) {
			assert.Equal(t, userID, n.UserID)
			assert.Equal(t, events.TypeDisputeLifecycle, n.Type)
			assert.Contains(t, n.Body, "dp-1")
			return true, nil
		})

	err = m.handleDelivery(context.Background(), amqp.Delivery{Body: body, RoutingKey: events.TypeDisputeLifecycle, MessageId: uuid.NewString()})
	assert.NoError(t, err)
}

func TestHandleDelivery_DisputeLifecycle_RedeliveryIsAckedAfterDedup(t *testing.T) {
	m, repo := newModule(t)
	m.config = DefaultConfig()
	m.renderer = notifytemplate.NewRenderer(0, 0)
	userID := uuid.New()
	ev := events.NewDisputeLifecycle(uuid.New(), uuid.New(), &userID, "dp-2", "mastercard", "", "700000", "IDR", "open", "lost", nil, time.Now().UTC())
	body, err := json.Marshal(ev)
	require.NoError(t, err)
