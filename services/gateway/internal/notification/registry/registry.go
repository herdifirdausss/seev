...
var kinds = map[string]Kind{
	...
	model.KindDisputeLifecycle: {
		Kind: model.KindDisputeLifecycle, Category: model.CategoryCompliance,
		Priority: model.PriorityHigh, Requirement: model.RequirementTransactional,
		AuthoritativeEvent: events.TypeDisputeLifecycle, TemplateSchemaVersion: 1,
		DigestEligible: false,
		DefaultModes:   map[string]string{model.ChannelInApp: model.ModeImmediate, model.ChannelEmail: model.ModeImmediate, model.ChannelPush: model.ModeImmediate},
		DeepLinkPath:   "/notifications", PrivacyClass: "financial", RetentionClass: "notification_delivery",
	},
...
// KindForDispute is the explicit Ledger-to-notification mapping for every
// chargeback lifecycle transition. It keeps dispute policy separate from the
// transaction-type registry so a new dispute state cannot accidentally inherit
// a money-movement template.
func KindForDispute() (Kind, error) {
	value, ok := Lookup(model.KindDisputeLifecycle)
	if !ok {
		return Kind{}, fmt.Errorf("notification: registry kind %q is missing", model.KindDisputeLifecycle)
	}
	return value, nil
}
