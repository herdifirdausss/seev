...
// Re-exported so callers never need to import services/payin/internal/payin/model.
...
func (m *Module) completeQuotedIntent(ctx context.Context, intent model.TopupIntent) (model.TopupIntent, error) {
	if intent.FeeQuoteID == nil {
		return model.TopupIntent{}, fmt.Errorf("payin: quoted intent is missing fee_quote_id")
	}
	...
	if !ok {
		return model.TopupIntent{}, fmt.Errorf("payin: fee snapshot recovery is unavailable")
	}
	txType := "money_in"
	if intent.MerchantTenantID != uuid.Nil {
		txType = "merchant_payin_credit"
	}
	...
	if !totalDebit.BigInt().IsInt64() {
		return model.TopupIntent{}, fmt.Errorf("payin: topup total debit exceeds int64")
	}
	if consumedAt == nil {
		return model.TopupIntent{}, fmt.Errorf("payin: consumed fee quote did not return a consumption timestamp")
	}
