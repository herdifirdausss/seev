...
// other code may import from services/payin — importing
// services/payin/internal/repository or services/payin/internal/payin/model directly from outside
...
import (
	...
	"github.com/herdifirdausss/seev/services/payin/internal/transport/grpc"
)

// Re-exported types so callers never need to import services/payin/internal/payin/model.
...
func (m *Module) HandleVendorCallback(ctx context.Context, vendor, vendorEventID, externalReference, amountRaw, currency, status, occurredAt, inboxID, requestID, unknownStatus string) (string, error) {
	...
	event := model.WebhookEvent{
		ID: identifiers.NewV7(), Vendor: vendor, VendorEventID: vendorEventID,
...
// services/ledger/internal/ledger/schedule's own isBusinessFailure (docs/roadmap/archive/19
