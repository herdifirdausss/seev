...
import (
	...
	"github.com/herdifirdausss/seev/services/payin"
	vendorboundary "github.com/herdifirdausss/seev/services/vendor-service"
)
...
func TestPayin_NormalizedCallback_IsIdempotent(t *testing.T) {
	...
	ctx := context.Background()
	userID := uuid.New()
	// The callback creates a user-owned money_in posting. Keep this fixture
	// aligned with the execution-time authorization contract instead of
	// bypassing it with a directly inserted account.
	validUntil := time.Now().UTC().Add(time.Hour)
	require.NoError(t, testutil.NewLedgerHarness(db).SetExecutionSubjectState(ctx, userID, "active", 1, &validUntil))
