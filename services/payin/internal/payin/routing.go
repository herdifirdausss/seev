...
import (
	...
	"github.com/herdifirdausss/seev/internal/platform/observability/metrics"
...
// (docs/roadmap/archive/40 Task T2) — mirrors services/payout's ResolvePayoutRoute.
func (m *Module) ResolveTopupRoute(ctx context.Context, userID uuid.UUID, currency string, amount decimal.Decimal) (string, string, error) {
	started := time.Now()
	result := "not_found"
	defer func() { metrics.ObserveResolution("payin", "payin_routing", result, started) }()
