...
import (
	...
	"github.com/herdifirdausss/seev/internal/platform/database/nulls"
...
// plaintext column anymore (services/payin/migrations/000010), so every write
...
func (r *repo) GetOrInsert(ctx context.Context, ev model.WebhookEvent) (model.WebhookEvent, error) {
	...
	_, err = r.db.ExecContext(ctx, `
		...
		ev.FeeQuoteConsumedAt, ev.FeeSnapshotVersion, nulls.String(ev.RequestID), rawCiphertext, v,
	)
	if err != nil {
...
func (r *repo) scanEvent(scanner interface{ Scan(...any) error }) (model.WebhookEvent, error) {
	...
	if feeGateway.Valid {
		ev.FeeGateway = feeGateway.String
	}
	if feeApplication.Valid {
		ev.FeeApplication = feeApplication.String
	}
	if feeQuoteConsumedAt.Valid {
		ev.FeeQuoteConsumedAt = &feeQuoteConsumedAt.Time
	}
	if feeQuoteID.Valid {
		parsed, err := uuid.Parse(feeQuoteID.String)
		if err != nil {
			return model.WebhookEvent{}, fmt.Errorf("parse payin webhook fee quote id: %w", err)
		}
	...
	if feeRuleID.Valid {
		...
		if err != nil {
			return model.WebhookEvent{}, fmt.Errorf("parse payin webhook fee rule id: %w", err)
		}
		ev.FeeRuleID = &parsed
	}
	if ev.TotalDebit.IsZero() {
