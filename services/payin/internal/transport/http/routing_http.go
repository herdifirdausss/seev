...
func (h *Handler) validateRulePayload(p routingRulePayload) (model.RoutingRule, error) {
...
func (h *Handler) createRoutingRuleHandler(w http.ResponseWriter, r *http.Request) {
	...
	rule, err := h.validateRulePayload(payload)
	if err != nil {
...
func (h *Handler) putVendorGatewayHandler(w http.ResponseWriter, r *http.Request) {
	...
	mapping, err := h.service.ValidateVendorGateway(vendor, payload.Gateway)
	if err != nil {
		response.BadRequest(w, err.Error())
		return
	}
