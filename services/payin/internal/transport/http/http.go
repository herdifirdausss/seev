...
// services/ledger/policy.Handler.Mux). Internal-router only; every handler is
...
func (h *Handler) AdminRouter() http.Handler {
	...
	mux.HandleFunc("GET /admin/payin/events", h.listEventsHandler)
	mux.HandleFunc("POST /admin/payin/events/{id}/replay", h.replayEventHandler)
	mux.HandleFunc("GET /admin/payin/routing-rules", h.listRoutingRulesHandler)
	mux.HandleFunc("POST /admin/payin/routing-rules", h.createRoutingRuleHandler)
	mux.HandleFunc("PUT /admin/payin/routing-rules/{id}", h.updateRoutingRuleHandler)
	mux.HandleFunc("GET /admin/payin/vendor-gateways/{vendor}", h.getVendorGatewayHandler)
	mux.HandleFunc("PUT /admin/payin/vendor-gateways/{vendor}", h.putVendorGatewayHandler)
	mux.HandleFunc("GET /admin/payin/vendors/health", h.vendorHealthHandler)
	mux.HandleFunc("POST /admin/payin/intake/pause", h.directPauseHandler)
	return mux
}
...
// the "/admin/payin/" subtree, see services/payin/cmd/payin/router.go; every
...
func (h *Handler) listEventsHandler(w http.ResponseWriter, r *http.Request) {
	...
	events, err := h.service.ListEvents(r.Context(), vendor, status, limit, offset)
	if err != nil {
		response.InternalServerError(w, err)
		return
	}
...
func (h *Handler) replayEventHandler(w http.ResponseWriter, r *http.Request) {
	if !isAdmin(r) {
		response.Forbidden(w, "admin privileges required")
		return
	}
	...
	if err := h.service.ReplayEvent(r.Context(), id); err != nil {
		switch {
