...
func (h *Handler) CreateClosureHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		closure, err := h.module.RequestClosure(r.Context(), userID, req.Password)
		if err != nil {
...
func (h *Handler) ClosureStatusHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		userID, ok := currentUserID(r)
		if !ok {
			response.Unauthorized(w, "invalid session")
		...
		closure, err := h.module.GetClosureStatus(r.Context(), userID, id)
		if err != nil {
			writeClosureError(w, err)
			return
		}
