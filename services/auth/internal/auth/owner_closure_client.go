...
// services/auth/cmd/auth/main.go wires via RegisterClosureOwner, once per
// owner. httpClient is expected to already carry the mTLS transport bound
// to the target owner's identity (internal/platform/security/tls.HTTPClient(certSrc, identity,
// timeout)) — this type adds only the Authorization: Bearer header
// internal/platform/security/middleware.WithInternalToken on each owner's side checks, the same
