...
// shape services/auth/cmd/auth/main.go wires in production, just without mTLS).
...
func TestClosure_NonZeroBalance_Blocks(t *testing.T) {
	...
	// The shared command boundary re-checks the ledger execution subject for
	// every user-owned posting. This fixture is intentionally funded before
	// closure, so establish the same approved subject state that a real KYC
	// approval would synchronize.
	validUntil := time.Now().UTC().Add(time.Hour)
	require.NoError(t, harness.SetExecutionSubjectState(ctx, userID, "active", 1, &validUntil))
...
func TestClosure_Commit_IdempotentUnderReplay(t *testing.T) {
	...
	firstCommitHash, firstDeletedCount, err := client.Commit(ctx, userID, surrogateID)
	require.NoError(t, err)
	require.Greater(t, firstDeletedCount, 0)

	secondCommitHash, secondDeletedCount, err := client.Commit(ctx, userID, surrogateID)
	require.NoError(t, err)
	require.Equal(t, firstCommitHash, secondCommitHash)
	require.Equal(t, firstDeletedCount, secondDeletedCount)
	...
	require.Equal(t, firstDeletedCount, accountCount, "no duplicate/extra rows must exist after replaying Commit")
}
...
func TestClosure_LedgerEntriesUnchanged(t *testing.T) {
	...
	validUntil := time.Now().UTC().Add(time.Hour)
	require.NoError(t, harness.SetExecutionSubjectState(ctx, alice, "active", 1, &validUntil))
	require.NoError(t, harness.SetExecutionSubjectState(ctx, bob, "active", 1, &validUntil))
