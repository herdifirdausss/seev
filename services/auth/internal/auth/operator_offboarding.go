...
func (m *Module) ApproveOperatorOffboarding(ctx context.Context, id uuid.UUID, approvedBy string) (OperatorOffboardingRequest, error) {
	...
	err = m.db.WithTx(ctx, nil, func(tx *sql.Tx) error {
		...
		userUpdateResult, err := tx.ExecContext(ctx, `UPDATE auth_users SET status = $1, updated_at = now() WHERE id = $2 AND status = $3`,
			model.StatusClosing, req.TargetUserID, model.StatusActive)
		if err != nil {
			return err
		}
		updatedUserCount, err := userUpdateResult.RowsAffected()
		if err != nil {
			return err
		}
	...
	if err := m.syncExecutionSubject(ctx, req.TargetUserID, model.StatusClosing, 0, nil); err != nil {
		return OperatorOffboardingRequest{}, fmt.Errorf("auth: synchronize offboarded execution subject: %w", err)
	}
