...
// Package auth_test drives services/auth.Module end to end against a real
...
func TestAuth_RegisterLoginMe_EndToEnd(t *testing.T) {
	...
	require.Equal(t, http.StatusOK, rec.Code, "issued JWT must pass internal/platform/security/middleware.WithAuth")
	...
	got, loginPair, err := m.Login(ctx, "ALICE@example.com", "hunter22!") // case-insensitive email
	require.NoError(t, err)
...
func TestAuth_RefreshRotation_RealRows(t *testing.T) {
	...
	_, rotatedPair, err := m.Refresh(ctx, pair.RefreshToken)
	require.NoError(t, err)
	...
	_, _, err = m.Refresh(ctx, rotatedPair.RefreshToken)
	assert.ErrorIs(t, err, auth.ErrInvalidRefreshToken,
