...
// Modeled on services/payout/internal/payout/relay.go's own dispatchOne discipline: every
...
// internal/platform/lifecycle/objectoutbox.Worker.ProcessOnce) so integration tests can drive it
...
// internal/platform/lifecycle/retention/worker's own holdsGauge.
...
// rows arrive pre-marshaled from the wire (services/auth.OwnerClosureClient.Export).
...
// convention services/auth/internal/auth/retention.go and object_outbox.go already use;
