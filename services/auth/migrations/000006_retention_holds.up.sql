...
-- Prefixed with the owning service (auth_) rather than the unprefixed
...
CREATE TABLE auth_retention_holds (
    id            UUID PRIMARY KEY,
    ...
    CONSTRAINT chk_auth_retention_hold_release_fields CHECK (
    ...
    CONSTRAINT chk_auth_retention_hold_releaser_not_creator CHECK (
        released_by IS NULL OR released_by <> created_by
    )
);

CREATE INDEX idx_auth_retention_holds_scope  ON auth_retention_holds(scope, scope_value) WHERE status = 'active';
CREATE INDEX idx_auth_retention_holds_expiry ON auth_retention_holds(expires_at) WHERE status = 'active' AND expires_at IS NOT NULL;
...
CREATE TABLE auth_retention_audit (
...
CREATE INDEX idx_auth_retention_audit_class ON auth_retention_audit(class, finished_at DESC);
CREATE INDEX idx_auth_retention_audit_job   ON auth_retention_audit(job_id);

GRANT SELECT, INSERT, UPDATE ON auth_retention_holds TO app_service;
GRANT SELECT, INSERT ON auth_retention_audit TO app_service;
GRANT SELECT ON auth_retention_holds, auth_retention_audit TO app_readonly;

ALTER TABLE auth_retention_holds ENABLE ROW LEVEL SECURITY;
ALTER TABLE auth_retention_audit ENABLE ROW LEVEL SECURITY;
ALTER TABLE auth_retention_holds FORCE ROW LEVEL SECURITY;
ALTER TABLE auth_retention_audit FORCE ROW LEVEL SECURITY;

CREATE POLICY auth_retention_holds_service  ON auth_retention_holds FOR ALL    TO app_service  USING (true) WITH CHECK (true);
CREATE POLICY auth_retention_holds_readonly ON auth_retention_holds FOR SELECT TO app_readonly USING (true);
CREATE POLICY auth_retention_audit_service  ON auth_retention_audit FOR ALL    TO app_service  USING (true) WITH CHECK (true);
CREATE POLICY auth_retention_audit_readonly ON auth_retention_audit FOR SELECT TO app_readonly USING (true);
...
CREATE OR REPLACE FUNCTION fn_auth_retention_hold_covers(
    p_table    TEXT,
    p_subject  TEXT,
    p_resource TEXT,
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = pg_catalog, public AS $$
    SELECT EXISTS (
        SELECT 1 FROM auth_retention_holds h
...
GRANT EXECUTE ON FUNCTION fn_auth_retention_hold_covers(TEXT, TEXT, TEXT, TIMESTAMPTZ) TO app_service;
