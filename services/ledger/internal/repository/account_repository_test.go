...
func TestGetSystemAccountID_SecondCall_HitsCacheNotDB(t *testing.T) {
	...
	firstLookupResult, err := repo.GetSystemAccountID(ctx, "settlement", "bca", "IDR")
	require.NoError(t, err)
	require.Equal(t, wantID, firstLookupResult)
	...
	secondLookupResult, err := repo.GetSystemAccountID(ctx, "settlement", "bca", "IDR")
	require.NoError(t, err)
	require.Equal(t, wantID, secondLookupResult)
...
func TestGetSystemAccountID_DifferentQualifier_MissesCache(t *testing.T) {
	...
	firstLookupResult, err := repo.GetSystemAccountID(ctx, "settlement", "bca", "IDR")
	require.NoError(t, err)
	require.Equal(t, bcaID, firstLookupResult)

	secondLookupResult, err := repo.GetSystemAccountID(ctx, "settlement", "gopay", "IDR")
	require.NoError(t, err)
	require.Equal(t, gopayID, secondLookupResult)
...
func TestGetSystemAccountID_DifferentCurrency_MissesCache(t *testing.T) {
	...
	firstLookupResult, err := repo.GetSystemAccountID(ctx, "settlement", "bca", "IDR")
	require.NoError(t, err)
	require.Equal(t, idrID, firstLookupResult)

	secondLookupResult, err := repo.GetSystemAccountID(ctx, "settlement", "bca", "USD")
	require.NoError(t, err)
	require.Equal(t, usdID, secondLookupResult)
	require.NotEqual(t, firstLookupResult, secondLookupResult)

	// Both must now be cache-served — no more query expectations queued.
	cachedIDRAccountID, err := repo.GetSystemAccountID(ctx, "settlement", "bca", "IDR")
	require.NoError(t, err)
	require.Equal(t, idrID, cachedIDRAccountID)
...
func TestGetAccountID_SecondCall_HitsCacheNotDB(t *testing.T) {
	...
	firstLookupResult, err := repo.GetAccountID(ctx, userID, "cash")
	require.NoError(t, err)
	require.Equal(t, wantID, firstLookupResult)

	secondLookupResult, err := repo.GetAccountID(ctx, userID, "cash")
	require.NoError(t, err)
	require.Equal(t, wantID, secondLookupResult)
...
func TestGetPocketAccountID_SecondCall_HitsCacheNotDB(t *testing.T) {
	...
	firstLookupResult, err := repo.GetPocketAccountID(ctx, userID, "travel")
	require.NoError(t, err)
	require.Equal(t, wantID, firstLookupResult)

	secondLookupResult, err := repo.GetPocketAccountID(ctx, userID, "travel")
	require.NoError(t, err)
	require.Equal(t, wantID, secondLookupResult)
...
func TestGetAccountCurrency_SecondCall_HitsCacheNotDB(t *testing.T) {
	...
	firstLookupResult, err := repo.GetAccountCurrency(ctx, accountID)
	require.NoError(t, err)
	require.Equal(t, "IDR", firstLookupResult)

	secondLookupResult, err := repo.GetAccountCurrency(ctx, accountID)
	require.NoError(t, err)
	require.Equal(t, "IDR", secondLookupResult)
