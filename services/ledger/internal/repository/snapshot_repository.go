...
func (r *snapshotRepo) InsertForDateAllAccounts(ctx context.Context, date time.Time) (int, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("insert complete C5 snapshot for %s: %w", date.Format(dateLayout), err)
	}
	n, err := res.RowsAffected()
	if err != nil {
		return 0, fmt.Errorf("insert complete C5 snapshot for %s: rows affected: %w", date.Format(dateLayout), err)
	}
	return int(n), nil
}
