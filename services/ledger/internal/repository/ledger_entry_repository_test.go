...
func TestInsertEntries_SingleRoundTrip_ForMultipleEntries(t *testing.T) {
	...
	debitAccountID, firstCreditAccountID, secondCreditAccountID := uuid.New(), uuid.New(), uuid.New()
	txID := uuid.New()
	entries := []model.EntryInstruction{
		{AccountID: debitAccountID, Direction: constant.Debit, Amount: decimal.NewFromInt(1000)},
		{AccountID: firstCreditAccountID, Direction: constant.Credit, Amount: decimal.NewFromInt(950)},
		{AccountID: secondCreditAccountID, Direction: constant.Credit, Amount: decimal.NewFromInt(50)},
	}
	newBalances := map[uuid.UUID]decimal.Decimal{
		debitAccountID:        decimal.NewFromInt(4000),
		firstCreditAccountID:  decimal.NewFromInt(950),
		secondCreditAccountID: decimal.NewFromInt(50),
	}
