...
func scanScheduledAttempt(scan func(dest ...any) error) (model.ScheduledExecutionAttempt, error) {
	...
	if transactionID.Valid {
		id, err := uuid.Parse(transactionID.String)
		if err != nil {
			return model.ScheduledExecutionAttempt{}, err
		}
		item.LedgerTransactionID = &id
...
func (r *scheduledOccurrenceRepo) ListAttempts(ctx context.Context, occurrenceID uuid.UUID) ([]model.ScheduledExecutionAttempt, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("list scheduled execution attempts: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if scanErr != nil {
			return nil, fmt.Errorf("scan scheduled execution attempt: %w", scanErr)
		}
		result = append(result, item)
	}
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("iterate scheduled execution attempts: %w", err)
	}
	return result, nil
}
