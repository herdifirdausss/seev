...
func resolveInlineFee(ctx context.Context, repo repository.AccountRepository, cmd Command, currency string) (feeID uuid.UUID, fee decimal.Decimal, err error) {
	...
	fee, err = metadata.MetaDecimal(cmd.Metadata, "fee_amount")
	if err != nil {
	...
	feeGateway, gErr := metadata.MetaString(cmd.Metadata, "fee_gateway")
...
func requireGateway(cmd Command, processor string) (string, error) {
	gw, err := metadata.MetaString(cmd.Metadata, "gateway")
	if err != nil || gw == "" {
...
func newPostedEvent(cmd ResolvedCommand, txID uuid.UUID, entries []model.EntryInstruction) model.OutboxEvent {
	...
	externalRef, _ := metadata.MetaString(cmd.Metadata, "external_ref")
	requestID, _ := metadata.MetaString(cmd.Metadata, "request_id")
	...
	if cmd.Type == "money_in" {
		if totalDebit, err := metadata.MetaDecimal(cmd.Metadata, "total_debit"); err == nil {
			feeAmount := decimal.Zero
			if fee, feeErr := metadata.MetaDecimal(cmd.Metadata, "fee_amount"); feeErr == nil {
				feeAmount = fee
			}
			ev.TotalDebit = totalDebit.String()
			ev.FeeAmount = feeAmount.String()
			if feeGateway, gatewayErr := metadata.MetaString(cmd.Metadata, "fee_gateway"); gatewayErr == nil {
				ev.FeeGateway = feeGateway
			}
			if feeApplication, applicationErr := metadata.MetaString(cmd.Metadata, "fee_application"); applicationErr == nil {
				ev.FeeApplication = feeApplication
			}
			if quoteID, quoteErr := metadata.MetaUUID(cmd.Metadata, "fee_quote_id"); quoteErr == nil {
				ev.FeeQuoteID = &quoteID
			}
			if payinID, payinErr := metadata.MetaUUID(cmd.Metadata, "payin_id"); payinErr == nil {
				ev.PayinID = &payinID
			}
		}
	}
...
func NewDefaultRegistry(
	...
	return NewRegistry(
		...
		// reachable only via services/payin and services/payout's own
