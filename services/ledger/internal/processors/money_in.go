...
func (p *MoneyIn) ResolveAccounts(ctx context.Context, cmd Command) (ResolvedAccounts, string, error) {
	...
	if cmd.PocketCode != "" {
		destID, err = userPocketAccountID(ctx, p.repo, cmd.UserID, cmd.PocketCode, cmd.Currency)
		if err != nil {
			return ResolvedAccounts{}, "", fmt.Errorf("money_in: pocket %q: %w", cmd.PocketCode, err)
		}
	} else {
		destID, err = userAccountID(ctx, p.repo, cmd.UserID, constant.AccountTypeCash, cmd.Currency)
		if err != nil {
	...
	if feeID, _, feeErr := resolveInlineFee(ctx, p.repo, cmd, currency); feeErr != nil {
		return ResolvedAccounts{}, "", fmt.Errorf("money_in: %w", feeErr)
...
func (p *MoneyIn) Validate(ctx context.Context, tx *sql.Tx, cmd ResolvedCommand, bal map[uuid.UUID]model.AccountBalance) error {
	...
	if application, _ := metadata.MetaString(cmd.Metadata, "fee_application"); application == "added_on_top" {
		totalDebit, err := metadata.MetaDecimal(cmd.Metadata, "total_debit")
		if err != nil {
