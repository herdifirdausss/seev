...
func TestResolvedAccounts_Reversal_SourceDestinationAlwaysNil(t *testing.T) {
	...
	refID := uuid.New()
	firstAccountID, secondAccountID := uuid.New(), uuid.New()
	txRepo.EXPECT().GetAccountIDs(gomock.Any(), refID).Return([]uuid.UUID{firstAccountID, secondAccountID}, nil)
	accRepo.EXPECT().GetAccountCurrency(gomock.Any(), firstAccountID).Return("IDR", nil)
	...
	assert.ElementsMatch(t, []uuid.UUID{firstAccountID, secondAccountID}, ra.Ordered)
}
