...
// services/ledger/internal/ledger/accrual BEFORE this processor ever runs, from a
...
// orchestrator/job-triggered (services/ledger/internal/worker/accrual.go), never a
...
func (p *InterestAccrue) BuildEntries(_ context.Context, _ *sql.Tx, cmd ResolvedCommand, _ map[uuid.UUID]model.AccountBalance) ([]model.EntryInstruction, error) {
	accrualDate, _ := metadata.MetaString(cmd.Metadata, "accrual_date")
	rateBps, _ := metadata.MetaString(cmd.Metadata, "rate_bps")
...
func (p *InterestAccrue) ValidateCommand(_ context.Context, cmd Command) error {
	...
	if v, err := metadata.MetaString(cmd.Metadata, "accrual_date"); err != nil || v == "" {
		return fmt.Errorf("%w: interest_accrue requires metadata 'accrual_date'", apperror.ErrValidation)
	}
	if v, err := metadata.MetaString(cmd.Metadata, "rate_bps"); err != nil || v == "" {
