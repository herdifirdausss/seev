...
// through the maker-checker flow (services/ledger/internal/ledger/adjustments), same
...
func (p *AdjustmentSuspenseCredit) ResolveAccounts(ctx context.Context, cmd Command) (ResolvedAccounts, string, error) {
	...
	currency, err := metadata.MetaString(cmd.Metadata, "currency")
	if err != nil || currency == "" {
	...
	if cmd.Currency != "" && currency != cmd.Currency {
		return ResolvedAccounts{}, "", fmt.Errorf("%w: adjustment_suspense_credit currency metadata does not match command currency", apperror.ErrCurrencyMismatch)
	}
...
func (p *AdjustmentSuspenseCredit) Validate(ctx context.Context, tx *sql.Tx, cmd ResolvedCommand, bal map[uuid.UUID]model.AccountBalance) error {
	if _, err := metadata.MetaString(cmd.Metadata, "authorized_by"); err != nil {
...
func (p *AdjustmentSuspenseCredit) BuildEntries(_ context.Context, _ *sql.Tx, cmd ResolvedCommand, _ map[uuid.UUID]model.AccountBalance) ([]model.EntryInstruction, error) {
	reason, _ := metadata.MetaString(cmd.Metadata, "reason")
