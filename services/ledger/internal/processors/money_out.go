package processors

import (
	"context"
	"database/sql"
	"fmt"

	"github.com/google/uuid"
	"github.com/herdifirdausss/seev/services/ledger/internal/ledger/constants"
	"github.com/herdifirdausss/seev/services/ledger/internal/ledger/model"
	"github.com/herdifirdausss/seev/services/ledger/internal/repository"
)

// =============================================================================
// 2. MoneyOut — Payment / Withdrawal request
//
// Source is user.cash by default; set cmd.PocketCode to debit from a pocket.
//
// Metadata:
//   "gateway"     (required) — selects settlement[gateway] as counterparty
//   "fee_amount"  (optional) — withdrawal fee / processing fee charged to user
//   "fee_gateway" (optional, required if fee_amount set)
//
// 2-entry (no fee):   user.cash|pocket  → settlement[gateway]
// 3-entry (with fee): user.cash|pocket  → settlement[gateway]   gross - fee
//                                       → fee[fee_gateway]       fee
//
// NOTE: No amount caps — enforce at API/policy layer.
//
// [0] = user.cash | user.pocket
// [1] = settlement[gateway]
// [2] = fee[fee_gateway]  (only when fee_amount > 0)
// =============================================================================

type MoneyOut struct{ repo repository.AccountRepository }

func NewMoneyOut(r repository.AccountRepository) *MoneyOut { return &MoneyOut{repo: r} }
func (p *MoneyOut) Type() string                           { return "money_out" }

func (p *MoneyOut) ResolveAccounts(ctx context.Context, cmd Command) (ResolvedAccounts, string, error) {
	gateway, err := requireGateway(cmd, "money_out")
	if err != nil {
		return ResolvedAccounts{}, "", err
	}
	var srcID uuid.UUID
	if cmd.PocketCode != "" {
		srcID, err = userPocketAccountID(ctx, p.repo, cmd.UserID, cmd.PocketCode, cmd.Currency)
		if err != nil {
			return ResolvedAccounts{}, "", fmt.Errorf("money_out: pocket %q: %w", cmd.PocketCode, err)
		}
	} else {
		srcID, err = userAccountID(ctx, p.repo, cmd.UserID, constant.AccountTypeCash, cmd.Currency)
		if err != nil {
			return ResolvedAccounts{}, "", fmt.Errorf("money_out: cash account: %w", err)
		}
	}
	currency, err := p.repo.GetAccountCurrency(ctx, srcID)
	if err != nil {
		return ResolvedAccounts{}, "", fmt.Errorf("money_out: currency: %w", err)
	}
	sysID, err := p.repo.GetSystemAccountID(ctx, constant.AccountTypeSettlement, gateway, currency)
	if err != nil {
		return ResolvedAccounts{}, "", fmt.Errorf("money_out: settlement[%s]: %w", gateway, err)
	}
	resolved := twoLeg(srcID, sysID)
	if feeID, _, feeErr := resolveInlineFee(ctx, p.repo, cmd, currency); feeErr != nil {
		return ResolvedAccounts{}, "", fmt.Errorf("money_out: %w", feeErr)
	} else if feeID != uuid.Nil {
		resolved.Ordered = append(resolved.Ordered, feeID)
	}
	return resolved, currency, nil
}

func (p *MoneyOut) Validate(ctx context.Context, tx *sql.Tx, cmd ResolvedCommand, bal map[uuid.UUID]model.AccountBalance) error {
	v := MultiValidator{
		PositiveAmountValidator{}, IntegralAmountValidator{},
		SufficientFundsValidator{AccountID: cmd.AccountIDs[0]},
	}
	if _, _, ok := hasFee(cmd); ok {
		v = append(v, FeeAmountValidator{})
	}
	return v.Validate(ctx, tx, cmd, bal)
}

func (p *MoneyOut) BuildEntries(_ context.Context, _ *sql.Tx, cmd ResolvedCommand, _ map[uuid.UUID]model.AccountBalance) ([]model.EntryInstruction, error) {
	srcNote := "cash"
	if cmd.PocketCode != "" {
		srcNote = "pocket:" + cmd.PocketCode
	}
	feeID, fee, withFee := hasFee(cmd)
	net := cmd.Amount
	if withFee {
		net = cmd.Amount.Sub(fee)
	}
	entries := []model.EntryInstruction{
		{AccountID: cmd.AccountIDs[0], Direction: constant.Debit, Amount: cmd.Amount, Note: "money_out ← " + srcNote},
		{AccountID: cmd.AccountIDs[1], Direction: constant.Credit, Amount: net},
	}
	if withFee {
		entries = append(entries, model.EntryInstruction{
			AccountID: feeID, Direction: constant.Credit, Amount: fee, Note: "money_out fee",
		})
	}
	return entries, nil
}

func (p *MoneyOut) OutboxEvents(cmd ResolvedCommand, txID uuid.UUID, entries []model.EntryInstruction) []model.OutboxEvent {
	return []model.OutboxEvent{newPostedEvent(cmd, txID, entries)}
}
func (p *MoneyOut) AfterCommit(_ context.Context, _ Command) error { return nil }

// ValidateCommand performs pre-DB validation (e.g. required metadata keys).
// No processor-specific requirements beyond what ResolveAccounts/Validate already check.
func (p *MoneyOut) ValidateCommand(_ context.Context, _ Command) error { return nil }
