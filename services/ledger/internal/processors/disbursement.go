...
// services/ledger/internal/ledger/disbursement, never a direct end-user action.
//
...
func (p *Disbursement) BuildEntries(_ context.Context, _ *sql.Tx, cmd ResolvedCommand, _ map[uuid.UUID]model.AccountBalance) ([]model.EntryInstruction, error) {
	batchID, _ := metadata.MetaString(cmd.Metadata, "batch_id")
	itemNo, _ := metadata.MetaString(cmd.Metadata, "item_no")
