...
func TestHandle_EmptyKey_Rejected(t *testing.T) {
	...
	defer ctrl.Finish()
	balanceRepo, balanceRepoController := newMockAccountBalanceRepo(t)
	defer balanceRepoController.Finish()
	entryRepo, entryRepoController := newMockEntryRepo(t)
	defer entryRepoController.Finish()
	outboxRepo, outboxRepoController := newMockOutboxRepo(t)
...
// =============================================================================
// metadata.MetaDecimal — float64 precision correctness  [FIX #1 iter2]
// =============================================================================

func TestMetaDecimal_StringInput_Exact(t *testing.T) {
	d, err := metadata.MetaDecimal(map[string]any{"v": "0.10"}, "v")
	require.NoError(t, err)
	assert.Equal(t, "0.10", d.StringFixed(2))
}

func TestMetaDecimal_MissingKey_Error(t *testing.T) {
	_, err := metadata.MetaDecimal(map[string]any{}, "v")
	require.Error(t, err)
}
