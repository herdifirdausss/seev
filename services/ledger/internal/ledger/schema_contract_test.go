...
// what the Go code in services/ledger actually reads and writes (docs/roadmap/archive/04
...
import (
	...
	"github.com/herdifirdausss/seev/services/ledger/internal/ledger/accrual"
	"github.com/herdifirdausss/seev/services/ledger/internal/ledger/adjustments"
	"github.com/herdifirdausss/seev/services/ledger/internal/ledger/disbursement"
	"github.com/herdifirdausss/seev/services/ledger/internal/ledger/dispute"
	...
	"github.com/herdifirdausss/seev/services/ledger/internal/ledger/recon"
...
// services/ledger/internal/ledger/handle's own unit tests) — screening is no
// longer part of this pipeline at all (docs/roadmap/archive/37): it moved to the
// transport layer, tested separately (services/ledger/internal/transport,
// contracts/clients/fraud).
...
func newDisputeService(db *database.DBSQL) (*dispute.Service, *ledgerhandle.Service, repository.AccountRepository) {
	handleSvc, accRepo := newService(db)
	disputeRepo := repository.NewChargebackDisputeRepositoryWithOutbox(db, repository.NewOutboxRepository(db))
	txRepo := repository.NewTransactionRepository(db, schemaTestDigestRing())
...
// services/ledger/internal/processors/chargeback.go never had on its own.
func TestSchemaContract_ChargebackDispute_FullLifecycle(t *testing.T) {
	...
	require.NoError(t, db.QueryRowContext(ctx, `
		...
		WHERE aggregate_type = 'chargeback_dispute' AND aggregate_id = $1
		  AND event_type = $2`, disputeID, events.TypeDisputeLifecycle).Scan(&lifecycleEvents))
	assert.Equal(t, 3, lifecycleEvents, "open, evidence_submitted, and lost must each have one transactional lifecycle event")
}
...
func TestSchemaContract_BalanceSnapshot_MultiDay(t *testing.T) {
	...
	firstDay := time.Date(2026, 6, 1, 10, 0, 0, 0, loc)
	secondDay := time.Date(2026, 6, 2, 10, 0, 0, 0, loc) // deliberately no activity
	thirdDay := time.Date(2026, 6, 3, 10, 0, 0, 0, loc)

	seedCreditEntry(t, db, funding, cashA, 10_000, 10_000, firstDay)
	seedCreditEntry(t, db, funding, cashA, 5_000, 15_000, thirdDay)
	...
	// cares about.
	firstDayRowCount, err := repo.InsertForDate(ctx, firstDay)
	require.NoError(t, err)
	require.Equal(t, 2, firstDayRowCount)

	secondDayRowCount, err := repo.InsertForDate(ctx, secondDay)
	require.NoError(t, err)
	require.Equal(t, 0, secondDayRowCount, "secondDay had no activity — must not write a row")

	thirdDayRowCount, err := repo.InsertForDate(ctx, thirdDay)
	require.NoError(t, err)
	require.Equal(t, 2, thirdDayRowCount)

	// GetLatestBefore(secondDay) must walk back to firstDay's snapshot — no row
	// exists for secondDay itself.
	bal, asOf, found, err := repo.GetLatestBefore(ctx, cashA, secondDay)
	require.NoError(t, err)
	require.True(t, found)
	require.True(t, bal.Equal(decimal.NewFromInt(10_000)))
	require.Equal(t, firstDay.Format("2006-01-02"), asOf.Format("2006-01-02"))

	// BalanceAsOf each snapshotted date matches the running balance.
	firstDayBalance, err := repo.BalanceAsOf(ctx, cashA, firstDay)
	require.NoError(t, err)
	require.True(t, firstDayBalance.Equal(decimal.NewFromInt(10_000)))

	thirdDayBalance, err := repo.BalanceAsOf(ctx, cashA, thirdDay)
	require.NoError(t, err)
	require.True(t, thirdDayBalance.Equal(decimal.NewFromInt(15_000)))

	// Re-running for firstDay is idempotent: no new row, no error, no double
	// counting.
	nAgain, err := repo.InsertForDate(ctx, firstDay)
	require.NoError(t, err)
	...
	require.NoError(t, db.QueryRowContext(ctx,
		...
		cashA, firstDay.Format("2006-01-02"),
	).Scan(&rowCount))
	require.Equal(t, 1, rowCount, "must still be exactly one snapshot row for firstDay")
}
...
func TestSchemaContract_BalanceSnapshot_TimezoneBoundary(t *testing.T) {
	...
	firstDay := time.Date(2026, 6, 10, 23, 30, 0, 0, loc) // 2026-06-10 23:30 WIB = 16:30 UTC same day
	secondDay := time.Date(2026, 6, 11, 0, 30, 0, 0, loc) // 2026-06-11 00:30 WIB = 17:30 UTC — SAME UTC day as above

	seedCreditEntry(t, db, funding, cashA, 1_000, 1_000, firstDay)
	seedCreditEntry(t, db, funding, cashA, 2_000, 3_000, secondDay)
	...
	secondDayRowCount, err := repo.InsertForDate(ctx, secondDay)
	require.NoError(t, err)
	require.Equal(t, 2, secondDayRowCount)

	var firstDayEntryCount, secondDayEntryCount int
	var firstDayClosingBalance, secondDayClosingBalance int64
	require.NoError(t, db.QueryRowContext(ctx,
		`SELECT entry_count, closing_balance FROM account_balance_snapshots WHERE account_id=$1 AND as_of_date=$2::date`,
		cashA, "2026-06-10",
	).Scan(&firstDayEntryCount, &firstDayClosingBalance))
	require.NoError(t, db.QueryRowContext(ctx,
		`SELECT entry_count, closing_balance FROM account_balance_snapshots WHERE account_id=$1 AND as_of_date=$2::date`,
		cashA, "2026-06-11",
	).Scan(&secondDayEntryCount, &secondDayClosingBalance))

	require.Equal(t, 1, firstDayEntryCount, "2026-06-10 (WIB) snapshot must count only its own entry")
	require.Equal(t, int64(1_000), firstDayClosingBalance)
	require.Equal(t, 1, secondDayEntryCount, "2026-06-11 (WIB) snapshot must count only its own entry")
	require.Equal(t, int64(3_000), secondDayClosingBalance)
}
...
func TestSchemaContract_BalanceSnapshot_MismatchDetected(t *testing.T) {
	...
	firstDay := time.Now().In(loc)
	seedCreditEntry(t, db, funding, cashA, 7_000, 7_000, firstDay)

	// 2, not 1 — see TestSchemaContract_BalanceSnapshot_MultiDay's own
	// comment: cashA and funding both have activity on this day now.
	n, err := repo.InsertForDate(ctx, firstDay)
	require.NoError(t, err)
	require.Equal(t, 2, n)

	// Consistent so far — VerifyDate must find nothing.
	clean, err := repo.VerifyDate(ctx, firstDay)
	require.NoError(t, err)
	...
	_, err = db.ExecContext(ctx,
		...
		999_999, cashA, firstDay.Format("2006-01-02"))
	require.NoError(t, err)

	mismatches, err := repo.VerifyDate(ctx, firstDay)
	require.NoError(t, err)
...
func TestSchemaContract_BalanceSnapshot_LatestDate_EmptyTable(t *testing.T) {
	...
	firstDay := time.Now().In(loc)
	seedCreditEntry(t, db, funding, cashA, 1_000, 1_000, firstDay)
	_, err = repo.InsertForDate(ctx, firstDay)
	require.NoError(t, err)

	date, found, err := repo.LatestSnapshotDate(ctx)
	require.NoError(t, err)
	require.True(t, found)
	require.Equal(t, firstDay.Format("2006-01-02"), date.Format("2006-01-02"))
}
...
// balance must come from a snapshot of firstDay, not a replay from account
// creation).
func TestSchemaContract_Statement_PeriodOpeningClosing(t *testing.T) {
	...
	firstDay := time.Date(2026, 5, 1, 10, 0, 0, 0, loc)
	secondDay := time.Date(2026, 5, 2, 10, 0, 0, 0, loc)
	thirdDay := time.Date(2026, 5, 3, 10, 0, 0, 0, loc)

	seedCreditEntry(t, db, funding, cashA, 10_000, 10_000, firstDay) // opening baseline
	seedCreditEntry(t, db, funding, cashA, 3_000, 13_000, secondDay) // in-period
	seedCreditEntry(t, db, funding, cashA, 2_000, 15_000, thirdDay)  // in-period

	_, err := snapshotRepo.InsertForDate(ctx, firstDay)
	require.NoError(t, err)

	// Statement period = [secondDay, thirdDay] — opening balance must come from
	// firstDay's snapshot (the day BEFORE the period), not a full replay back
	// to account creation.
	opening, err := snapshotRepo.BalanceAsOf(ctx, cashA, firstDay)
	require.NoError(t, err)
	require.True(t, opening.Equal(decimal.NewFromInt(10_000)), "opening balance must equal firstDay's closing balance")

	entries, err := entryRepo.ListByAccountRange(ctx, cashA, secondDay, thirdDay, loc, 100)
	require.NoError(t, err)
	require.Len(t, entries, 2, "period must contain exactly secondDay and thirdDay's entries, not firstDay's")
...
func TestSchemaContract_Statement_RangeTooLarge_LimitPlusOne(t *testing.T) {
	...
	const maxRows = 5
	firstDay := time.Date(2026, 5, 10, 8, 0, 0, 0, loc)
	balance := int64(0)
	for i := 0; i < maxRows+1; i++ {
		balance += 100
		seedCreditEntry(t, db, funding, cashA, 100, balance, firstDay.Add(time.Duration(i)*time.Minute))
	}

	entries, err := entryRepo.ListByAccountRange(ctx, cashA, firstDay, firstDay, loc, maxRows+1)
	require.NoError(t, err)
...
func TestSchemaContract_PendingAdjustment_RetryApprove_NoDoublePost(t *testing.T) {
	...
	approvalTransactionID, err := adjSvc.Approve(ctx, id, "approver-1")
	require.NoError(t, err)
	...
	require.Equal(t, approvalTransactionID, storedTxID)
}
...
// repository and feeding it into internal/platform/money/currency.Load makes currency.IsValid
...
// RunDue(firstDay) posts; RunDue(firstDay) again is idempotent (no double post);
// RunDue(secondDay) posts a second time.
func TestSchemaContract_Schedule_DailyRunDue_IdempotentAcrossDays(t *testing.T) {
	...
	firstDay := dateOnly(time.Now())
	id, err := svc.Create(ctx, userA, "transfer_p2p", decimal.NewFromInt(1_000), userB, "", nil, "daily", firstDay, nil, userA.String())
	require.NoError(t, err)

	executed, failed, err := svc.RunDue(ctx, firstDay)
	require.NoError(t, err)
	...
	require.Equal(t, 1, countLedgerTransactions(t, db, scheduleIdempotencyKeyForTest(id, firstDay)))
	...
	executed, failed, err = svc.RunDue(ctx, secondDay)
	require.NoError(t, err)
	assert.Equal(t, 1, executed)
	assert.Equal(t, 0, failed)
	...
	require.Equal(t, 1, countLedgerTransactions(t, db, scheduleIdempotencyKeyForTest(id, secondDay)))
}
...
func TestSchemaContract_Schedule_ListDue_PerScheduleKind(t *testing.T) {
	...
	asOf := dateOnly(time.Now())
	if asOf.Day() > 28 {
		// day_of_month is CHECK'd to [1,28] (services/ledger/migrations/000014_scheduled_transactions.up.sql)
...
func TestSchemaContract_Disbursement_ImportThenRun_AllPostedAcrossMultipleCalls(t *testing.T) {
	...
	firstRunResult, err := svc.Run(ctx, batchID, false)
	require.NoError(t, err)
	assert.Equal(t, 6, firstRunResult.Processed)
	assert.Equal(t, 6, firstRunResult.Posted)
	assert.False(t, firstRunResult.Done, "6 of 10 processed — batch must not be done yet")

	secondRunResult, err := svc.Run(ctx, batchID, false)
	require.NoError(t, err)
	assert.Equal(t, 4, secondRunResult.Processed)
	assert.Equal(t, 4, secondRunResult.Posted)
	assert.True(t, secondRunResult.Done)
...
// TestSchemaContract_Disbursement_RawSQLCannotSkipApproval proves the
// database invariant independently of the Go service. This catches a future
// repository or migration script that accidentally updates status directly.
func TestSchemaContract_Disbursement_RawSQLCannotSkipApproval(t *testing.T) {
	db := setupSchemaTestDB(t)
	ctx := context.Background()
	id := uuid.New()
	_, err := db.ExecContext(ctx, `INSERT INTO disbursement_batches (id, source_filename, row_count, status, created_by) VALUES ($1, 'raw.csv', 0, 'pending_approval', 'maker')`, id)
	require.NoError(t, err)
	_, err = db.ExecContext(ctx, `UPDATE disbursement_batches SET status='processing' WHERE id=$1`, id)
	require.Error(t, err, "raw SQL must not enter processing without approval metadata")
	_, err = db.ExecContext(ctx, `UPDATE disbursement_batches SET status='processing', approved_by='checker', approved_at=now() WHERE id=$1`, id)
	require.NoError(t, err)
	_, err = db.ExecContext(ctx, `UPDATE disbursement_batches SET approved_by='another-checker' WHERE id=$1`, id)
	require.Error(t, err, "approval attribution must be immutable after processing starts")
}
...
func TestSchemaContract_Disbursement_Resume_NoDoublePost(t *testing.T) {
	...
	alreadyPostedItemKey := "batch:" + batchID.String() + ":5"
	require.NoError(t, handleSvc.Handle(ctx, processors.Command{
		IdempotencyKey: alreadyPostedItemKey,
		Type:           "disbursement",
		Amount:         decimal.NewFromInt(500),
		UserID:         users[4],
	}))
	...
	require.NoError(t, err)
	var alreadyPostedItemID uuid.UUID
	for _, it := range items {
		if it.ItemNo == 5 {
			alreadyPostedItemID = it.ID
		}
	}
	require.NotEqual(t, uuid.Nil, alreadyPostedItemID)
	postedTx, err := repository.NewTransactionRepository(db, schemaTestDigestRing()).GetByIdempotencyKey(ctx, alreadyPostedItemKey, nil)
	require.NoError(t, err)
	require.NoError(t, db.WithTx(ctx, nil, func(tx *sql.Tx) error {
		return disbRepo.MarkItemPosted(ctx, tx, alreadyPostedItemID, postedTx.ID)
	}))
...
func TestSchemaContract_Disbursement_BusinessFailure_OtherItemsStillProcess(t *testing.T) {
	...
	firstGoodUserID := uuid.New()
	firstGoodCashID := createUserCashAccount(t, db, firstGoodUserID)
	badUser := uuid.New() // never provisioned — no cash account
	secondGoodUserID := uuid.New()
	secondGoodCashID := createUserCashAccount(t, db, secondGoodUserID)

	rows := []model.DisbursementImportRow{
		{UserID: firstGoodUserID, Amount: decimal.NewFromInt(1_000)},
		{UserID: badUser, Amount: decimal.NewFromInt(2_000)},
		{UserID: secondGoodUserID, Amount: decimal.NewFromInt(3_000)},
	}
	...
	require.True(t, getBalance(t, db, firstGoodCashID).Equal(decimal.NewFromInt(1_000)))
	require.True(t, getBalance(t, db, secondGoodCashID).Equal(decimal.NewFromInt(3_000)))
...
// services/ledger/internal/ledger/handle for a caller to even configure.
// Screening now happens entirely in the transport layer, before Handle is
// ever called (see services/ledger/internal/transport's own block/fail-open tests).
