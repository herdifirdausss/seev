...
func TestOpenDispute_CurrencyMismatch_Rejected(t *testing.T) {
	repo, ctrl := newMockDisputeRepo(t)
	defer ctrl.Finish()
	s := New(repo, &fakeTxReader{tx: model.LedgerTransaction{Status: "posted", Currency: "IDR", Amount: decimal.NewFromInt(50_000)}})
...
func TestOpenDispute_AmountAboveOriginal_Rejected(t *testing.T) {
	repo, ctrl := newMockDisputeRepo(t)
	defer ctrl.Finish()
	s := New(repo, &fakeTxReader{tx: model.LedgerTransaction{Status: "posted", Currency: "IDR", Amount: decimal.NewFromInt(100)}})
	_, err := s.OpenDispute(context.Background(), uuid.New(), "dp-too-large", "visa", "", decimal.NewFromInt(101), "IDR", nil, "ops-1")
	require.Error(t, err)
	assert.ErrorIs(t, err, apperror.ErrValidation)
}
...
func TestResolveDispute_Success(t *testing.T) {
	...
	defer ctrl.Finish()
	id := uuid.New()
	repo.EXPECT().GetDispute(gomock.Any(), id).Return(model.ChargebackDispute{ID: id, CreatedBy: "maker-1", Status: "evidence_submitted"}, nil)
...
func TestResolveDispute_CreatorCannotApproveOwnResolution(t *testing.T) {
	repo, ctrl := newMockDisputeRepo(t)
	defer ctrl.Finish()
	id := uuid.New()
	repo.EXPECT().GetDispute(gomock.Any(), id).Return(model.ChargebackDispute{ID: id, CreatedBy: "ops-1", Status: "evidence_submitted"}, nil)

	s := New(repo, &fakeTxReader{})
	err := s.ResolveDispute(context.Background(), id, "lost", "ops-1", "network ruled against us")
	require.Error(t, err)
	assert.ErrorIs(t, err, apperror.ErrValidation)
}
