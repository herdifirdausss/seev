...
// event publishing.
//
// This is the ONLY package other modules or services/gateway/cmd/gateway may import from
// services/ledger — importing any subpackage (repository, processors,
...
import (
	...
	"github.com/herdifirdausss/seev/services/ledger/internal/ledger/closure"
	...
	fxservice "github.com/herdifirdausss/seev/services/ledger/internal/ledger/fx"
	...
	"github.com/herdifirdausss/seev/services/ledger/internal/migration/balancev2"
	...
	"github.com/herdifirdausss/seev/services/ledger/internal/transport"
	"github.com/herdifirdausss/seev/services/ledger/internal/transport/grpc"
	"github.com/herdifirdausss/seev/services/ledger/internal/worker"
)

// Re-exported types so callers never need to import services/ledger
...
type (
	...
	// without importing services/ledger/internal/transport; they never need to
	// import services/ledger/policy either, since Go interface satisfaction is
	...
	// caller outside this module (e.g. services/payin, docs/roadmap/archive/22 Task
	// T2) can classify "business failure, won't heal on retry" vs "infra
	// error, retry/redeliver" via errors.As(err, &ledgerErr), the same
	// pattern services/ledger/internal/ledger/schedule already uses internally.
	LedgerError = apperror.LedgerError
	// Quote is a fee quote row (docs/roadmap/archive/38) — re-exported so a caller
	// outside this module (e.g. internal/testkit's LedgerHarness, used by
	// services/payout's own integration tests to create a quote to consume)
	// can name CreateQuote's return type without importing the
	// module-private services/ledger/internal/feepolicy package directly.
	Quote = feepolicy.Quote
	// ChargebackDispute is one card-network dispute case-management record
	// (business-completeness audit finding — services/ledger/migrations/000035_
	...
	// services/ledger/migrations/000037_chargeback_dispute_audit_trail).
...
// a caller outside this module (e.g. services/payout, docs/roadmap/archive/23 Task T4)
...
// needing to import the module-private services/ledger/internal/feepolicy package.
...
// re-exported so a caller outside this module (internal/testkit's
// LedgerHarness, standing in for a real gRPC-connected ledgerclient.Client
// in services/gateway/internal/merchant/client's own integration tests) can classify
...
// integrity verifier). Deliberately independent of internal/platform/config — the
// module must not depend on the composition root's config type.
type WorkerConfig struct {
	Enabled bool
	// FinancialProductsEnabled is an explicit activation switch for durable occurrences and
	// monthly interest workers. The new tables and operator controls can be
	// deployed before any product behavior is scheduled.
	FinancialProductsEnabled bool
...
type Module struct {
	...
	executionSubjects repository.ExecutionSubjectRepository
	...
	financialProductsJob *worker.FinancialProductJob
	retentionRunner      *retentionworker.Runner
	retentionSched       *scheduler.Scheduler
	disputeExpiryJob     *worker.DisputeExpiryJob
	stuckStateScanner    *worker.StuckStateScanner
...
// services/ledger/internal/ledger/handle entirely, into the transport layer, so no
...
func NewModule(db database.DatabaseSQL, broker messaging.Broker, redisClient *redis.Client, workerCfg WorkerConfig, logger *slog.Logger, maxAmountPerTx decimal.Decimal, policyChecker PolicyChecker, fraudClient *fraudcheck.Client, feeQuoteTTL time.Duration, digestRing *cryptox.DigestRing, cryptoxRing *cryptox.Ring) *Module {
	...
	executionSubjects := repository.NewExecutionSubjectRepository(db)
	policyDecisionRepo := repository.NewPolicyDecisionRepository(db)
	disputeRepo := repository.NewChargebackDisputeRepositoryWithOutbox(db, outboxRepo)

	feeRepo := repository.NewFeeRepository(db)
	feeApproval := feepolicy.NewApproval(repository.NewFeeRuleApprovalRepository(db))
	...
	// threaded through internal/platform/config or NewModule's own (already long)
	...
	executor := commandservice.NewExecutor(handleSvc, policyChecker, commandservice.SubjectAuthorizer{Reader: executionSubjects}, policyDecisionRepo, logger)
	adjustmentsSvc := adjustments.New(db, adjRepo, txRepo, outboxRepo, executor)
	scheduleSvc := schedule.New(db, scheduleRepo, executor, logger)
	durableScheduleSvc := schedule.NewDurable(scheduleRepo, scheduleOccurrenceRepo, executor, feeQuotePolicy, logger, loc)
	scheduleSvc.SetExecutor(executor)
	durableScheduleSvc.SetExecutor(executor)
	durableScheduleSvc.SetDatabase(db)
	durableScheduleSvc.SetOutbox(outboxRepo)
	if workerCfg.FinancialProductsEnabled {
		scheduleSvc.SetDurable(durableScheduleSvc)
	}
	disbursementSvc := disbursement.New(db, disbursementRepo, txRepo, executor)
	accrualSvc := accrual.New(db, savingsRepo, snapshotRepo, executor, logger)
	interestSvc := interestservice.New(db, interestRepo, snapshotRepo, executor, logger, loc)
	...
	m := &Module{
		...
		executor:               executor,
		executionSubjects:      executionSubjects,
	}
	...
	m.feeApproval = feeApproval
	m.router = transport.NewRouterWithFraudAndExecutorAndFeeApproval(m, policyChecker, m.feePolicy, fraudClient, logger, feeQuoteTTL, executor, feeApproval)
	...
	m.financialProductsJob = worker.NewFinancialProductJob(m, lock, logger, loc)
	m.disputeExpiryJob = worker.NewDisputeExpiryJob(m.disputeSvc, lock, logger, loc)
	m.stuckStateScanner = worker.NewStuckStateScanner(repository.NewStuckStateRepository(db), lock, logger, loc, alertFn)
...
// (rather than only reachable via HTTP) so internal/testkit's
// LedgerHarness can create quotes for integration tests (e.g.
// services/payout's own) without any caller needing to import the
// module-private services/ledger/internal/feepolicy package — see the Quote
// re-export above.
...
// grpcserver's gRPC mapping AND internal/testkit's in-process harness — can
...
// import, which services/ledger/internal/feepolicy being module-private forbids.
...
// SetExecutionSubjectState synchronizes the small auth projection used by the
// command executor. It is intentionally additive to ApplyKycTier: KYC limits
// and authorization state are separate controls and must not be conflated.
func (m *Module) SetExecutionSubjectState(ctx context.Context, userID uuid.UUID, status string, kycLevel int, verifiedUntil *time.Time) error {
	if m.executionSubjects == nil {
		return errors.New("ledger execution subject repository is not configured")
	}
	return m.executionSubjects.UpsertExecutionSubject(ctx, model.ExecutionSubject{
		UserID: userID, Status: status, KYCLevel: kycLevel,
		KYCVerifiedUntil: verifiedUntil, TenantStatus: "active",
	})
}
...
func (m *Module) InternalRouter() http.Handler {
	return transport.NewInternalRouterWithExecutorAndFeeApproval(m, m.feePolicy, m.executor, m.feeApproval)
}
...
// this in internal/platform/security/middleware.WithInternalToken and mount it only on the
...
// LoadCurrencies loads the `currencies` table into internal/platform/money/currency's runtime
...
func (m *Module) StartWorkers(ctx context.Context) {
	...
	if m.workerCfg.FinancialProductsEnabled {
		if err := m.financialProductsJob.Start(ctx); err != nil {
			m.logger.Error("ledger: failed to start C5 financial product job", slog.Any("error", err))
		}
	}
	if err := m.disputeExpiryJob.Start(ctx); err != nil {
		m.logger.Error("ledger: failed to start dispute expiry job", slog.Any("error", err))
	}
	if err := m.stuckStateScanner.Start(ctx); err != nil {
		m.logger.Error("ledger: failed to start stuck-state scanner", slog.Any("error", err))
	}
...
func (m *Module) StopWorkers() {
	...
	m.accrualJob.Stop()
	if m.workerCfg.FinancialProductsEnabled {
		m.financialProductsJob.Stop()
	}
	if m.disputeExpiryJob != nil {
		m.disputeExpiryJob.Stop()
	}
	if m.stuckStateScanner != nil {
		m.stuckStateScanner.Stop()
	}
...
func (m *Module) Post(ctx context.Context, cmd Command) error {
	if m.executor == nil {
		return errors.New("ledger command executor is not configured")
	}
	return m.executor.Execute(ctx, cmd, commandservice.ExecutionContext{
		Source: "module-api", RequestOrigin: "ledger.Module.Post", TenantID: cmd.MerchantTenantID,
	})
}
...
func (m *Module) ProvisionUser(ctx context.Context, userID uuid.UUID, currency string) ([]Account, error) {
	accounts, err := m.provisionSvc.CreateUserAccounts(ctx, userID, currency)
	if err != nil {
		return nil, err
	}
	// Account provisioning is also the ledger's first opportunity to establish
	// the fail-closed execution subject used by public money-movement commands.
	...
	// E2E journey (docs/roadmap/archive/60-c4-end-to-end-multi-currency.md).
...
// services/payout, docs/roadmap/archive/23 Task T3) recovers the tx ID Post() itself
...
// the schedule — enforced in services/ledger/internal/ledger/schedule.
...
func (m *Module) SetSavingsProductStatus(ctx context.Context, id uuid.UUID, status, checker string) (SavingsProduct, error) {
	...
	product, err := m.interestRepo.GetProduct(ctx, id)
	if err != nil {
	...
	if err := m.interestRepo.UpdateProductStatus(ctx, id, status, checker); err != nil {
		return SavingsProduct{}, err
	}
	return m.interestRepo.GetProduct(ctx, id)
}
...
func (m *Module) CreateSavingsRate(ctx context.Context, rate SavingsRateVersion) (SavingsRateVersion, error) {
	...
	return m.interestRepo.CreateRate(ctx, rate)
}
...
func (m *Module) SubmitSavingsRate(ctx context.Context, id uuid.UUID, maker string) error {
	...
	return m.interestRepo.SubmitRate(ctx, id, maker)
}
...
func (m *Module) ApproveSavingsRate(ctx context.Context, id uuid.UUID, checker string) error {
	...
	return m.interestRepo.ApproveRate(ctx, id, checker)
}
...
func (m *Module) RejectSavingsRate(ctx context.Context, id uuid.UUID, checker, reason string) error {
	...
	return m.interestRepo.RejectRate(ctx, id, checker, reason)
}
...
func (m *Module) EnrollSavingsAccount(ctx context.Context, enrollment SavingsEnrollment) (SavingsEnrollment, error) {
	...
	product, err := m.interestRepo.GetProduct(ctx, enrollment.ProductID)
	if err != nil {
...
func (m *Module) GetInterestPeriod(ctx context.Context, id uuid.UUID) (InterestPeriod, error) {
	return m.interestRepo.GetPeriod(ctx, id)
}
...
func (m *Module) updateSavingsEnrollmentStatus(ctx context.Context, id uuid.UUID, status, checker string, effectiveUntil *time.Time) error {
	...
	enrollment, err := m.interestRepo.GetEnrollment(ctx, id)
	if err != nil {
		return err
	}
	...
	if updater, ok := m.interestRepo.(interface {
	...
	updater, ok := m.interestRepo.(interface {
