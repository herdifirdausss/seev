...
func TestSchemaContract_OutboxBackoff_MarkFailedSetsExponentialDelay(t *testing.T) {
	...
	// [120s, 180s].
	secondAttemptStartedAt := time.Now()
	require.NoError(t, outboxRepo.MarkFailed(ctx, id, "publish timeout again"))
	secondFailure := getOutboxEvent(t, db, id)

	require.Equal(t, 2, secondFailure.RetryCount)
	require.NotNil(t, secondFailure.NextAttemptAt)
	secondRetryDelay := secondFailure.NextAttemptAt.Sub(secondAttemptStartedAt)
	require.GreaterOrEqual(t, secondRetryDelay, 119*time.Second, "delay too short for retry_count=2")
	require.LessOrEqual(t, secondRetryDelay, 185*time.Second, "delay too long for retry_count=2")
	require.Greater(t, secondRetryDelay, delay, "delay must grow with retry_count (exponential backoff)")
}
