...
func (s *Service) Import(ctx context.Context, filename string, rows []model.DisbursementImportRow, createdBy string) (uuid.UUID, error) {
	...
	for i, r := range rows {
		items[i] = model.DisbursementItem{
			ID: identifiers.NewV7(), BatchID: batchID, ItemNo: i + 1,
...
func (s *Service) Run(ctx context.Context, batchID uuid.UUID, retryFailed bool) (model.DisbursementRunResult, error) {
	...
	for _, item := range items {
		...
		actorID, _ := uuid.Parse(batch.ApprovedBy)
		postErr := commandservice.Run(ctx, s.poster, cmd, commandservice.ExecutionContext{
			ActorID: actorID, Source: "bulk-disbursement", CorrelationID: batchID.String(),
			RequestOrigin: "approved-disbursement-batch", Currency: item.Currency,
		})
