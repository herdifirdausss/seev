...
func (s *DurableService) SetExecutor(executor CommandExecutor) { s.executor = executor }
...
func (s *DurableService) ExecuteOccurrence(ctx context.Context, occurrenceID uuid.UUID, owner string) (bool, error) {
	...
	var postErr error
	if s.executor != nil {
		postErr = s.executor.Execute(ctx, postCommand, commandservice.ExecutionContext{
			ActorID: scheduleRow.UserID, Source: "scheduler", CorrelationID: occurrence.IdempotencyKey,
			RequestOrigin: "durable-schedule-occurrence:" + occurrence.ID.String(), Currency: currency,
		})
	} else {
		postErr = commandservice.Run(ctx, s.poster, postCommand, commandservice.ExecutionContext{
			ActorID: scheduleRow.UserID, Source: "scheduler", CorrelationID: occurrence.IdempotencyKey,
			RequestOrigin: "durable-schedule-occurrence:" + occurrence.ID.String(), Currency: currency,
		})
	}
