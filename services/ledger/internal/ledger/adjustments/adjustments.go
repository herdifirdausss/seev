...
// redefinitions rather than depending on internal/platform/database directly.
...
// admin JWT (services/ledger/internal/transport/http.go's adminOnlyTypes) despite
...
func (s *Service) Create(ctx context.Context, requestedBy, adjType string, amount decimal.Decimal, targetUserID, referenceID uuid.UUID, metadata map[string]any, reason string) (uuid.UUID, error) {
	...
	currencyCode := ""
	if rawCurrency, rawErr := ledgermetadata.MetaString(metadata, "currency"); rawErr == nil && strings.TrimSpace(rawCurrency) != "" {
	...
	case suspenseTypes[adjType]:
		gateway, _ := ledgermetadata.MetaString(metadata, "gateway")
	...
	case fxRebalanceTypes[adjType]:
		pair, _ := ledgermetadata.MetaString(metadata, "pair")
...
func (s *Service) Approve(ctx context.Context, id uuid.UUID, approverID string) (uuid.UUID, error) {
	...
	actorID, _ := uuid.Parse(approverID)
	if postErr := commandservice.Run(ctx, s.poster, cmd, commandservice.ExecutionContext{
		ActorID: actorID, Source: "admin-correction", CorrelationID: id.String(),
		RequestOrigin: "pending-adjustment-approval", Currency: payload.Currency,
	}); postErr != nil {
