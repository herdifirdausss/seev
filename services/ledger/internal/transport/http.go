...
// caller (the composition root, services/gateway/internal/transport/http/router.go) is responsible
...
func NewRouterWithFraud(svc Service, policy PolicyChecker, feePolicy *feepolicy.Policy, fraudClient *fraudcheck.Client, logger *slog.Logger, feeQuoteTTL time.Duration) http.Handler {
	return NewRouterWithFraudAndExecutor(svc, policy, feePolicy, fraudClient, logger, feeQuoteTTL, nil)
}

// NewRouterWithFraudAndExecutor is the production constructor. Policy checks
// are executed by the shared command boundary after this handler has decoded
// and normalized the request; this transport does not own authorization.
func NewRouterWithFraudAndExecutor(svc Service, policy PolicyChecker, feePolicy *feepolicy.Policy, fraudClient *fraudcheck.Client, logger *slog.Logger, feeQuoteTTL time.Duration, executor *commandservice.Executor) http.Handler {
	return NewRouterWithFraudAndExecutorAndFeeApproval(svc, policy, feePolicy, fraudClient, logger, feeQuoteTTL, executor, nil)
}

func NewRouterWithFraudAndExecutorAndFeeApproval(svc Service, policy PolicyChecker, feePolicy *feepolicy.Policy, fraudClient *fraudcheck.Client, logger *slog.Logger, feeQuoteTTL time.Duration, executor *commandservice.Executor, feeApproval *feepolicy.Approval) http.Handler {
	if logger == nil {
		logger = slog.Default()
	}
	h := &handler{svc: svc, allowedTypes: publicUserTypes, feePolicy: feePolicy, policy: policy, executor: executor, feeApproval: feeApproval, fraudClient: fraudClient, logger: logger, feeQuoteTTL: feeQuoteTTL}
	return h.mux()
}
...
// services/gateway/internal/transport/http/router.go NewInternalRouter and services/gateway/cmd/gateway/main.go). It
...
func NewInternalRouterWithFeePolicy(svc Service, feePolicy *feepolicy.Policy) http.Handler {
	return NewInternalRouterWithExecutorAndFeePolicy(svc, feePolicy, nil)
}

// NewInternalRouterWithExecutorAndFeePolicy keeps trusted internal routes on
// the same command boundary. The executor identifies this source as an
// internal service, so end-user velocity limits are not applied twice.
func NewInternalRouterWithExecutorAndFeePolicy(svc Service, feePolicy *feepolicy.Policy, executor *commandservice.Executor) http.Handler {
	return NewInternalRouterWithExecutorAndFeeApproval(svc, feePolicy, executor, nil)
}

func NewInternalRouterWithExecutorAndFeeApproval(svc Service, feePolicy *feepolicy.Policy, executor *commandservice.Executor, feeApproval *feepolicy.Approval) http.Handler {
	h := &handler{svc: svc, allowedTypes: nil, feePolicy: feePolicy, executor: executor, feeApproval: feeApproval}
	return h.mux()
}
...
func (h *handler) mux() http.Handler {
	...
	// services/ledger/internal/ledger/schedule), same as listAccounts/getBalance above.
	...
	mux.HandleFunc("GET /savings/products", h.listSavingsProducts)
	mux.HandleFunc("GET /savings/enrollments", h.listSavingsEnrollments)
	mux.HandleFunc("GET /savings/enrollments/{id}", h.getSavingsEnrollment)
	mux.HandleFunc("GET /savings/enrollments/{id}/accruals", h.listInterestAccruals)
	mux.HandleFunc("GET /savings/enrollments/{id}/periods", h.listInterestPeriods)
	...
	if h.allowedTypes == nil {
		...
		mux.HandleFunc("POST /admin/savings/products", h.createSavingsProduct)
		mux.HandleFunc("GET /admin/savings/products", h.listSavingsProducts)
		mux.HandleFunc("POST /admin/savings/products/{id}/status", h.updateSavingsProductStatus)
		mux.HandleFunc("POST /admin/savings/products/{id}/rates", h.createSavingsRate)
		mux.HandleFunc("POST /admin/savings/rates/{id}/submit", h.submitSavingsRate)
		mux.HandleFunc("POST /admin/savings/rates/{id}/approve", h.approveSavingsRate)
		mux.HandleFunc("POST /admin/savings/rates/{id}/reject", h.rejectSavingsRate)
		mux.HandleFunc("POST /admin/savings/enrollments", h.createSavingsEnrollment)
		mux.HandleFunc("POST /admin/savings/enrollments/{id}/pause", h.pauseSavingsEnrollment)
		mux.HandleFunc("POST /admin/savings/enrollments/{id}/resume", h.resumeSavingsEnrollment)
		mux.HandleFunc("POST /admin/savings/enrollments/{id}/end", h.endSavingsEnrollment)
		mux.HandleFunc("GET /admin/savings/periods/{id}/preview", h.previewInterestPeriod)
		mux.HandleFunc("POST /admin/savings/periods/{id}/close", h.closeInterestPeriod)
		mux.HandleFunc("POST /admin/savings/interest/run", h.runInterestDaily)
		mux.HandleFunc("POST /admin/savings/period-items/{id}/retry", h.retryInterestPeriodItem)
		mux.HandleFunc("POST /admin/savings/adjustments", h.createInterestAdjustment)
		mux.HandleFunc("POST /admin/savings/adjustments/{id}/approve", h.approveInterestAdjustment)
		...
		mux.HandleFunc("GET /admin/ledger/fee-rules/{id}/versions", h.listFeeRuleVersions)
		mux.HandleFunc("POST /admin/ledger/fee-rules", h.createFeeRule)
		mux.HandleFunc("PUT /admin/ledger/fee-rules/{id}", h.updateFeeRule)
		mux.HandleFunc("POST /admin/ledger/fee-rules/{id}/submit", h.submitFeeRule)
		mux.HandleFunc("POST /admin/ledger/fee-rules/{id}/approve", h.approveFeeRule)
		mux.HandleFunc("POST /admin/ledger/fee-rules/{id}/reject", h.rejectFeeRule)

		// Chargeback case management is Ledger-owned. These routes never post
		// money directly; any chargeback transaction remains a separate,
		// audited command.
		mux.HandleFunc("POST /admin/disputes", h.openDispute)
		mux.HandleFunc("GET /admin/disputes", h.listDisputes)
		mux.HandleFunc("GET /admin/disputes/{id}", h.getDispute)
		mux.HandleFunc("GET /admin/disputes/{id}/status-changes", h.listDisputeStatusChanges)
		mux.HandleFunc("POST /admin/disputes/{id}/evidence", h.submitDisputeEvidence)
		mux.HandleFunc("POST /admin/disputes/{id}/resolve", h.resolveDispute)
...
func (h *handler) createFeeRule(w http.ResponseWriter, r *http.Request) {
	...
	if h.feeApproval != nil {
		actor, actorOK := currentUserID(r)
		if !actorOK {
			response.Unauthorized(w, "invalid or missing admin identity")
			return
		}
		version, createErr := h.feeApproval.CreateDraft(r.Context(), rule, actor.String(), req.Reason)
		if createErr != nil {
			writeError(w, createErr)
			return
		}
		response.Accepted(w, feeRuleVersionResponse(version))
		return
	}
	created, err := h.feePolicy.Create(r.Context(), rule)
	if err != nil {
		writeError(w, err)
		return
	}
	response.Created(w, toFeeRuleResponse(created))
}
...
func (h *handler) updateFeeRule(w http.ResponseWriter, r *http.Request) {
	...
	updated, err := h.feePolicy.Update(r.Context(), rule)
	...
	response.OK(w, toFeeRuleResponse(updated))
}

func feeRuleVersionResponse(version model.FeeRuleVersion) map[string]any {
	return map[string]any{
		"id": version.ID, "rule_id": version.RuleID, "version": version.Version,
		"tx_type": version.TxType, "gateway": version.Gateway, "currency": version.Currency,
		"user_id": version.UserID, "flat_minor_units": version.FlatMinorUnits,
		"percent_basis_pts": version.PercentBasisPts, "fee_gateway": version.FeeGateway,
		"enabled": version.Enabled, "status": version.Status, "created_by": version.CreatedBy,
		"submitted_by": version.SubmittedBy, "approved_by": version.ApprovedBy,
		"rejected_by": version.RejectedBy, "decision_reason": version.DecisionReason,
		"effective_from": version.EffectiveFrom, "effective_until": version.EffectiveUntil,
		"created_at": version.CreatedAt, "updated_at": version.UpdatedAt,
	}
}

func (h *handler) listFeeRuleVersions(w http.ResponseWriter, r *http.Request) {
	if !isAdmin(r) || h.feeApproval == nil {
		response.Forbidden(w, "fee-rule approval is not configured")
		return
	}
	id, err := uuid.Parse(r.PathValue("id"))
	if err != nil {
	...
	items, err := h.feeApproval.ListVersions(r.Context(), id)
	if err != nil {
		writeError(w, err)
		return
	}
	...
	for _, item := range items {
		result = append(result, feeRuleVersionResponse(item))
	}
	response.OK(w, map[string]any{"versions": result})
}

func (h *handler) feeRuleAction(w http.ResponseWriter, r *http.Request, action string) {
	if !isAdmin(r) || h.feeApproval == nil {
		response.Forbidden(w, "fee-rule approval is not configured")
		return
	}
	id, err := uuid.Parse(r.PathValue("id"))
	if err != nil {
		response.BadRequest(w, "invalid fee-rule version id")
		return
	}
	...
	if r.ContentLength != 0 && !response.Decode(w, r, &body) {
		return
	}
	var version model.FeeRuleVersion
	switch action {
	case "submit":
		version, err = h.feeApproval.Submit(r.Context(), id, actor.String())
	case "approve":
		if !isAdminChecker(r) {
			response.Forbidden(w, "checker privileges required")
			return
		}
		version, err = h.feeApproval.Approve(r.Context(), id, actor.String(), body.Reason)
	case "reject":
		if !isAdminChecker(r) {
			response.Forbidden(w, "checker privileges required")
			return
		}
		version, err = h.feeApproval.Reject(r.Context(), id, actor.String(), body.Reason)
	default:
		response.BadRequest(w, "unknown fee-rule action")
		return
	}
	if err != nil {
		writeError(w, err)
	...
	response.OK(w, feeRuleVersionResponse(version))
}

func (h *handler) submitFeeRule(w http.ResponseWriter, r *http.Request) {
	h.feeRuleAction(w, r, "submit")
}

func (h *handler) approveFeeRule(w http.ResponseWriter, r *http.Request) {
	h.feeRuleAction(w, r, "approve")
}

func (h *handler) rejectFeeRule(w http.ResponseWriter, r *http.Request) {
	h.feeRuleAction(w, r, "reject")
}

type chargebackDisputeAPI interface {
	OpenChargebackDispute(context.Context, uuid.UUID, string, string, string, decimal.Decimal, string, *time.Time, string) (uuid.UUID, error)
	GetChargebackDispute(context.Context, uuid.UUID) (model.ChargebackDispute, error)
	ListOpenChargebackDisputes(context.Context, int, int) ([]model.ChargebackDispute, error)
	SubmitChargebackDisputeEvidence(context.Context, uuid.UUID, string, string) error
	ResolveChargebackDispute(context.Context, uuid.UUID, string, string, string) error
	ListChargebackDisputeStatusChanges(context.Context, uuid.UUID) ([]model.ChargebackDisputeStatusChange, error)
}

func (h *handler) disputes(w http.ResponseWriter) (chargebackDisputeAPI, bool) {
	api, ok := h.svc.(chargebackDisputeAPI)
	if !ok {
		response.ServiceUnavailable(w, "DISPUTES_UNAVAILABLE", "dispute case management is not configured")
		return nil, false
	}
	return api, true
}

func disputeResponse(d model.ChargebackDispute) map[string]any {
	return map[string]any{
		"id": d.ID, "original_tx_id": d.OriginalTxID, "chargeback_tx_id": d.ChargebackTxID,
		"dispute_ref": d.DisputeRef, "card_network": d.CardNetwork, "reason_code": d.ReasonCode,
		"amount": d.Amount.String(), "currency": d.Currency, "status": d.Status,
		"evidence_due_at": d.EvidenceDueAt, "evidence_ref": d.EvidenceRef,
		"resolved_at": d.ResolvedAt, "resolved_by": d.ResolvedBy,
		"resolution_reason": d.ResolutionReason, "created_by": d.CreatedBy,
...
func (h *handler) openDispute(w http.ResponseWriter, r *http.Request) {
	...
	originalID, err := uuid.Parse(req.OriginalTxID)
	if err != nil {
		response.BadRequest(w, "original_tx_id must be a valid UUID")
		return
	}
	amount, err := decimalFromString(req.Amount)
	if err != nil {
		response.BadRequest(w, "amount must be an integer minor-unit decimal string")
		return
	}
	var dueAt *time.Time
	if req.EvidenceDueAt != "" {
		parsed, parseErr := time.Parse(time.RFC3339, req.EvidenceDueAt)
		if parseErr != nil {
			response.BadRequest(w, "evidence_due_at must be RFC3339")
			return
		}
		dueAt = &parsed
	}
	id, err := api.OpenChargebackDispute(r.Context(), originalID, req.DisputeRef, req.CardNetwork, req.ReasonCode, amount, req.Currency, dueAt, actor.String())
	if err != nil {
		writeError(w, err)
		return
	}
...
func (h *handler) listDisputes(w http.ResponseWriter, r *http.Request) {
	...
	items, err := api.ListOpenChargebackDisputes(r.Context(), limit, offset)
	if err != nil {
		writeError(w, err)
		return
	}
	...
	for _, item := range items {
		out = append(out, disputeResponse(item))
	}
	response.OK(w, map[string]any{"disputes": out})
}
...
func (h *handler) getDispute(w http.ResponseWriter, r *http.Request) {
	...
	id, err := uuid.Parse(r.PathValue("id"))
	if err != nil {
		response.BadRequest(w, "invalid dispute id")
		return
	}
	d, err := api.GetChargebackDispute(r.Context(), id)
	if err != nil {
		writeError(w, err)
		return
	}
	response.OK(w, disputeResponse(d))
}

func (h *handler) listDisputeStatusChanges(w http.ResponseWriter, r *http.Request) {
	if !isAdmin(r) {
		response.Forbidden(w, "admin privileges required")
		return
	}
	...
	items, err := api.ListChargebackDisputeStatusChanges(r.Context(), id)
	if err != nil {
		writeError(w, err)
		return
	}
	response.OK(w, map[string]any{"status_changes": items})
}
...
func (h *handler) submitDisputeEvidence(w http.ResponseWriter, r *http.Request) {
	...
	if err := api.SubmitChargebackDisputeEvidence(r.Context(), id, req.EvidenceRef, actor.String()); err != nil {
		writeError(w, err)
		return
	}
	response.OK(w, map[string]any{"status": "evidence_submitted"})
}
...
func (h *handler) resolveDispute(w http.ResponseWriter, r *http.Request) {
	...
	if err := api.ResolveChargebackDispute(r.Context(), id, req.Status, actor.String(), req.Reason); err != nil {
		writeError(w, err)
		return
	}
	response.OK(w, map[string]any{"status": req.Status})
}
...
type handler struct {
	...
	// executor is the production money-movement boundary. Legacy constructors
	// leave it nil for isolated transport tests and compatibility embedders.
	executor    *commandservice.Executor
	feeApproval *feepolicy.Approval
...
// PolicyChecker is satisfied structurally by services/ledger/policy.Engine —
// defined here, not imported from there, so the ledger module never
// depends on services/ledger/policy (docs/roadmap/archive/17 Task T1, decision K-S S1:
...
// claims already validated by internal/platform/security/middleware.WithAuth.
...
func (h *handler) postTransaction(w http.ResponseWriter, r *http.Request) {
	...
	// Resolve only the currency needed to construct the execution context. The
	// policy decision itself belongs to command.Executor, never to transport.
	policyCurrency := req.Currency
	if (h.policy != nil || h.executor != nil) && policyCurrency == "" {
	...
	if h.executor != nil {
		...
		if claims != nil {
			claimValues["role"] = claims.Role
			claimValues["kyc_level"] = strconv.Itoa(claims.KYCLevel)
		}
		if err := h.executor.Execute(r.Context(), cmd, commandservice.ExecutionContext{
			ActorID: userID, Source: source, CorrelationID: middleware.RequestIDFromCtx(r.Context()),
			AuthorizationClaims: claimValues, RequestOrigin: r.Method + " " + r.URL.Path,
		...
		// Compatibility constructors used by narrow transport tests retain the
		// historical policy seam. Production wiring always supplies executor.
		if h.policy != nil {
			var allowed bool
			var rule, detail string
			var policyErr error
			if checker, ok := h.policy.(currencyPolicyChecker); ok && policyCurrency != "" {
				allowed, rule, detail, policyErr = checker.CheckWithCurrency(r.Context(), userID, req.Type, amount, policyCurrency)
			} else {
				allowed, rule, detail, policyErr = h.policy.Check(r.Context(), userID, req.Type, amount)
			}
			if policyErr != nil {
				response.InternalServerError(w, policyErr)
				return
			}
			if !allowed {
...
// (ownership enforced in services/ledger/internal/ledger/schedule, same
...
func (h *handler) createSchedule(w http.ResponseWriter, r *http.Request) {
	...
	var id uuid.UUID
	var createErr error
	financialProductScheduleRequested := req.Currency != "" || req.Timezone != "" || req.LocalTime != "" || req.MissedRunPolicy != "" || req.CatchUpLimit != 0 || req.MaxFeeAmount != nil || req.MaxInfrastructureAttempts != 0 || req.RetryWindowSeconds != 0 || req.FeeMode != "" || req.ConsecutiveFailureThreshold != 0
	if financialProductScheduleRequested {
		creator, ok := h.svc.(scheduleCreator)
		if !ok {
			if req.Currency == "" {
