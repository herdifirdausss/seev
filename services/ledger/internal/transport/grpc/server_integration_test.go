...
// directly rather than through internal/testkit.NewLedgerHarness.
func testDigestRing(t *testing.T) *cryptox.DigestRing {
	t.Helper()
	key := make([]byte, 32)
	for i := range key {
		key[i] = byte(i + 23)
...
func TestPostMoneyInEndToEndOverGRPC(t *testing.T) {
	...
	require.NoError(t, module.SetExecutionSubjectState(ctx, userID, "active", 1, &validUntil))
	...
	require.NoError(t, err)
	validUntil = time.Now().UTC().Add(time.Hour)
	require.NoError(t, module.SetExecutionSubjectState(ctx, userB, "active", 1, &validUntil))
	_, err = client.Post(ctx, &ledgerv1.PostRequest{
