...
// generic error: it lives here (services/fraud/internal/fraud/model), not in
// services/fraud itself, specifically so services/fraud/internal/transport/grpc can
// check errors.Is against it without importing services/fraud (which
// would cycle back, since services/fraud imports services/fraud/internal/transport/grpc
// to register the RPC service). grpcserver maps this to a distinguishable
// gRPC status; every caller (ledger/payin/payout, via contracts/clients/fraud) must
