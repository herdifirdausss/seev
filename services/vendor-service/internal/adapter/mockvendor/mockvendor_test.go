...
func TestVerifyAndParse_SignatureIsOverRawBytes_NotReMarshaledJSON(t *testing.T) {
	...
	reorderedEvent, reorderedErr := v.VerifyAndParse(headers, reordered)
	assert.Nil(t, reorderedEvent)
	assert.True(t, errors.Is(reorderedErr, vendorgw.ErrInvalidSignature))
}
...
// in services/payin, so a wrong/hardcoded name would misattribute
...
func TestVerifyAndParse_SecondNamedInstance_TagsEventWithOwnNameAndSecret(t *testing.T) {
	const secondVendorSecret = "second-vendor-secret"
	secondVendor := New("mockvendor2", secondVendorSecret)

	userID := uuid.New()
	body := validSettledBody(userID)

	ev, err := secondVendor.VerifyAndParse(signedRequest(t, secondVendorSecret, body), body)
	require.NoError(t, err)
	require.NotNil(t, ev)
	...
	_, err = secondVendor.VerifyAndParse(signedRequest(t, testSecret, body), body)
	assert.ErrorIs(t, err, vendorgw.ErrInvalidSignature, "mockvendor's secret must not verify mockvendor2's signature")

	defaultVendor := New(VendorName, testSecret)
	_, err = defaultVendor.VerifyAndParse(signedRequest(t, secondVendorSecret, body), body)
