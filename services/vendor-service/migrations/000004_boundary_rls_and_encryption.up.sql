...
-- (services/vendor-service/migrations/000002), gets ENABLE+FORCE ROW LEVEL SECURITY as
-- defense-in-depth against grant creep (see services/ledger/migrations/000009_rls_roles.up.sql's
...
-- services/vendor-service/migrations/000001_vendor_boundary.up.sql's own GRANT already uses,
...
-- perspective (grep confirms services/vendor-service/internal/callback.go's Ensure()
...
-- services/ledger/migrations/000035_chargeback_disputes.down.sql); rows already past
-- their 30-day redaction window would have been overwritten with a fixed
-- '{"redacted":true}' marker anyway (services/vendor-service/migrations/000003), so this only
