...
-- Prefixed with the owning service (payout_) rather than the unprefixed
...
CREATE TABLE payout_retention_holds (
    id            UUID PRIMARY KEY,
    ...
    CONSTRAINT chk_payout_retention_hold_release_fields CHECK (
    ...
    CONSTRAINT chk_payout_retention_hold_releaser_not_creator CHECK (
...
CREATE TABLE payout_retention_audit (
...
CREATE INDEX idx_payout_retention_audit_class ON payout_retention_audit(class, finished_at DESC);
CREATE INDEX idx_payout_retention_audit_job   ON payout_retention_audit(job_id);

GRANT SELECT, INSERT, UPDATE ON payout_retention_holds TO app_service;
GRANT SELECT, INSERT ON payout_retention_audit TO app_service;
GRANT SELECT ON payout_retention_holds, payout_retention_audit TO app_readonly;

ALTER TABLE payout_retention_holds ENABLE ROW LEVEL SECURITY;
ALTER TABLE payout_retention_audit ENABLE ROW LEVEL SECURITY;
ALTER TABLE payout_retention_holds FORCE ROW LEVEL SECURITY;
ALTER TABLE payout_retention_audit FORCE ROW LEVEL SECURITY;

CREATE POLICY payout_retention_holds_service  ON payout_retention_holds FOR ALL    TO app_service  USING (true) WITH CHECK (true);
CREATE POLICY payout_retention_holds_readonly ON payout_retention_holds FOR SELECT TO app_readonly USING (true);
...
CREATE OR REPLACE FUNCTION fn_payout_retention_hold_covers(
    p_table    TEXT,
    p_subject  TEXT,
    p_resource TEXT,
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = pg_catalog, public AS $$
    SELECT EXISTS (
        SELECT 1 FROM payout_retention_holds h
...
GRANT EXECUTE ON FUNCTION fn_payout_retention_hold_covers(TEXT, TEXT, TEXT, TIMESTAMPTZ) TO app_service;
