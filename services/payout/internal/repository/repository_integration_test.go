...
func TestTransitionToHeld_WrongStartingStatus_NoOp(t *testing.T) {
	...
	secondTxID := uuid.New()
	secondTransitionWon, err := repo.TransitionToHeld(ctx, req.ID, secondTxID)
	require.NoError(t, err)
	assert.False(t, secondTransitionWon, "a transition from the wrong starting status must be a no-op, not an error")
