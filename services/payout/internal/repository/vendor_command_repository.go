...
// services/ledger/internal/repository.OutboxRepository's claim/retry/reap/replay
...
func insertVendorCommand(ctx context.Context, tx *sql.Tx, payoutRequestID uuid.UUID, vendor string, attempt int) error {
	_, err := tx.ExecContext(ctx, `
		...
		identifiers.NewV7(), vendorCommandKey(payoutRequestID, attempt), payoutRequestID, vendor, attempt,
	)
	if err != nil {
...
// services/ledger/internal/repository/outbox_event_repository.go's MarkFailed: base
