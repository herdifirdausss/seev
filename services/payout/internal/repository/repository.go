...
// plaintext column anymore (services/payout/migrations/000011), so every write
...
func destinationAAD(requestID uuid.UUID) cryptox.AAD {
...
func (r *repo) Insert(ctx context.Context, req model.PayoutRequest) error {
	...
	_, err = r.db.ExecContext(ctx, `
		...
		nulls.String(req.RequestID), destCiphertext, v,
	)
	if err != nil {
...
func (r *repo) InsertMerchant(ctx context.Context, req model.PayoutRequest) (model.PayoutRequest, error) {
	...
	_, err = r.db.ExecContext(ctx, `
		...
		req.ID, req.UserID, req.MerchantTenantID, req.Amount.IntPart(), req.Currency, req.Vendor, req.CreatedBy,
		nulls.String(req.RequestID), req.DownstreamKey, destCiphertext, v,
	)
	if err != nil {
...
// established pattern as services/ledger/internal/repository's closed_by_tx_id
// (uuid.UUID's Scan method doesn't reliably handle a NULL through a bare
// **uuid.UUID destination).
func (r *repo) scanRequest(s rowScanner) (model.PayoutRequest, error) {
