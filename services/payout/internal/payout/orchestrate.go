...
func (m *Module) recordVendorCall(ctx context.Context, requestID uuid.UUID, summary string, result vendorgw.PayoutResult, callErr error, outcome string) error {
	call := model.PayoutVendorCall{
		ID: identifiers.NewV7(), PayoutRequestID: requestID, Attempt: 1, ReqSummary: summary, Outcome: outcome,
	}
	if callErr != nil {
...
//
// Called by the resume job (services/payout/internal/worker) on a tight interval,
