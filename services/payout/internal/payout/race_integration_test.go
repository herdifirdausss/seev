...
import (
	...
	"github.com/herdifirdausss/seev/contracts/vendorgw/mockvendor"
...
func activatePayoutExecutionSubject(t *testing.T, ledgerModule *testutil.LedgerHarness, userID uuid.UUID) {
	t.Helper()
	validUntil := time.Now().UTC().Add(time.Hour)
	require.NoError(t, ledgerModule.SetExecutionSubjectState(context.Background(), userID, "active", 1, &validUntil))
}
...
func setupHeldRequest(t *testing.T, db *database.DBSQL, m *Module, ledgerModule *testutil.LedgerHarness, amount int64) (model.PayoutRequest, uuid.UUID) {
	...
	activatePayoutExecutionSubject(t, ledgerModule, userID)
	...
	req := model.PayoutRequest{
		ID: identifiers.NewV7(), UserID: userID, Amount: decimal.NewFromInt(amount / 2), Currency: "IDR",
