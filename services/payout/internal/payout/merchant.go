...
// may ever be routed to (Plan 57 T6) — mirrors services/payin's own
...
// services/payin's CreateMerchantTopupIntent.
//
...
func (m *Module) CreateMerchant(ctx context.Context, tenantID uuid.UUID, environment, currency string, amount decimal.Decimal, destination []byte, createdBy, downstreamKey string) (uuid.UUID, error) {
	...
	// (Plan 57 T6 scope decision) — same reasoning as services/payin's own
...
// mirrors services/payin's own resolveMerchantVendor exactly. sandbox:
