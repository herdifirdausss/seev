...
// Package payout_test drives services/payout.Module.Create end to end
...
import (
	...
	"github.com/herdifirdausss/seev/services/payout"
...
// convention as services/payin's own integration tests) and a real
...
func setupFundedUser(t *testing.T, ledgerModule *testutil.LedgerHarness, amount int64) uuid.UUID {
	...
	require.NoError(t, ledgerModule.SetExecutionSubjectState(ctx, userID, "active", 1, &validUntil))
