...
// payout package (not services/payout/internal/worker) because dispatching a
// command needs settle/cancel/recordVendorCall/ResolvePayoutRoute, all
// unexported Module methods; services/payout/internal/worker only ever gets a thin
// ticker loop that calls back in through DispatchPendingCommands etc. (same
// split as services/payout/internal/worker.ResumeJob calling back through
