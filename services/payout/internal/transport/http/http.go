...
// services/ledger/internal/transport's own helper of the same name.
...
// services/ledger/internal/transport's own (unexported, cross-module-inaccessible)
...
func (h *Handler) CreateHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		id, err := h.service.Create(r.Context(), userID, amount, req.Destination, userID.String(), "")
		if err != nil {
			switch {
...
// services/payin.Module.AdminRouter). Internal-router only; every handler
...
func (h *Handler) AdminRouter() http.Handler {
	...
	mux.HandleFunc("GET /admin/payout/requests", h.listRequestsHandler)
	mux.HandleFunc("POST /admin/payout/requests/{id}/cancel", h.adminCancelHandler)
	mux.HandleFunc("POST /admin/payout/requests/{id}/retry", h.adminRetryHandler)
	mux.HandleFunc("GET /admin/payout/routing-rules", h.listRoutingRulesHandler)
	mux.HandleFunc("POST /admin/payout/routing-rules", h.createRoutingRuleHandler)
	mux.HandleFunc("PUT /admin/payout/routing-rules/{id}", h.updateRoutingRuleHandler)
	mux.HandleFunc("GET /admin/payout/vendor-gateways/{vendor}", h.getVendorGatewayHandler)
	mux.HandleFunc("PUT /admin/payout/vendor-gateways/{vendor}", h.putVendorGatewayHandler)
	mux.HandleFunc("POST /admin/payout/vendors/{vendor}/force-fail", h.vendorForceFailHandler)
	mux.HandleFunc("GET /admin/payout/vendors/health", h.vendorHealthHandler)
	mux.HandleFunc("GET /admin/payout/vendor-commands/dead", h.listDeadCommandsHandler)
	mux.HandleFunc("POST /admin/payout/vendor-commands/dead/{id}/replay", h.replayDeadCommandHandler)
	mux.HandleFunc("POST /admin/payout/vendor-commands/dead/replay-all", h.replayAllDeadCommandsHandler)
	mux.HandleFunc("POST /admin/payout/intake/pause", h.directPauseHandler)
	return mux
}
...
func (h *Handler) directPauseHandler(w http.ResponseWriter, r *http.Request) {
	claims := middleware.GetClaims(r.Context())
	...
	result, err := h.service.ApplyIntakeControl(r.Context(), commandID, "pause", request.ExpectedRevision, actor, request.Reason)
	if err != nil {
...
func (h *Handler) listDeadCommandsHandler(w http.ResponseWriter, r *http.Request) {
	if !isAdmin(r) {
	...
	commands, err := h.service.ListDeadVendorCommands(r.Context(), limit, offset)
	if err != nil {
		response.InternalServerError(w, err)
		return
	}
...
func (h *Handler) replayDeadCommandHandler(w http.ResponseWriter, r *http.Request) {
	if !isAdmin(r) {
		response.Forbidden(w, "admin privileges required")
		return
	}
	...
	if err := h.service.ReplayDeadVendorCommand(r.Context(), id); err != nil {
		if errors.Is(err, service.ErrVendorCommandNotFound) {
...
func (h *Handler) replayAllDeadCommandsHandler(w http.ResponseWriter, r *http.Request) {
	if !isAdmin(r) {
		response.Forbidden(w, "admin privileges required")
		return
	}
	...
	count, err := h.service.ReplayAllDeadVendorCommands(r.Context(), olderThan)
	if err != nil {
		response.InternalServerError(w, err)
		return
	}
...
// Task T5 — see services/payin/internal/transport/http/http.go's own vendorHealthHandler doc
...
// list.
func (h *Handler) vendorHealthHandler(w http.ResponseWriter, r *http.Request) {
	if !isAdmin(r) {
		response.Forbidden(w, "admin privileges required")
		return
	}
	vendors := h.service.VendorHealth(r.Context())
...
func (h *Handler) vendorForceFailHandler(w http.ResponseWriter, r *http.Request) {
	if !isAdmin(r) {
		response.Forbidden(w, "admin privileges required")
		return
	}
	...
	if err := h.service.ForceFailVendor(vendor, body.Fail); err != nil {
		switch {
		case errors.Is(err, service.ErrUnknownVendor):
			response.NotFound(w, "vendor not registered")
		case errors.Is(err, service.ErrForceFailUnsupported):
...
func (h *Handler) listRequestsHandler(w http.ResponseWriter, r *http.Request) {
	...
	reqs, err := h.service.List(r.Context(), status, vendor, limit, offset)
	if err != nil {
		response.InternalServerError(w, err)
		return
	}
...
func (h *Handler) adminCancelHandler(w http.ResponseWriter, r *http.Request) {
	if !isAdmin(r) {
		response.Forbidden(w, "admin privileges required")
		return
	}
	...
	if err := h.service.AdminCancel(r.Context(), id, body.Reason); err != nil {
		switch {
...
func (h *Handler) adminRetryHandler(w http.ResponseWriter, r *http.Request) {
	if !isAdmin(r) {
		response.Forbidden(w, "admin privileges required")
		return
	}
	...
	if err := h.service.AdminRetry(r.Context(), id); err != nil {
		switch {
		case errors.Is(err, service.ErrRequestNotFound):
			response.NotFound(w, "payout request not found")
		case errors.Is(err, ErrInvalidTransition):
