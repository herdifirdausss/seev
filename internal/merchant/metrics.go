...
// pkg/retentionworker/internal/auth's own "refreshed once per worker
// tick" convention (internal/auth/privacy_metrics.go). Labels are
...
var b2bAPIEnabledGauge = promauto.NewGauge(prometheus.GaugeOpts{
	...
	Help:      "1 if the global merchant B2B API kill switch (internal/merchant/auth.GlobalFlag) is enabled, 0 if an operator has disabled it.",
})
