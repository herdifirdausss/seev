...
// pkg/response.Decode already uses elsewhere in this codebase.
const maxRequestBodyBytes = 1 << 20

// currencyPattern mirrors api/openapi/components/common.yaml's Currency
// schema exactly (`^[A-Z]{3}$`).
var currencyPattern = regexp.MustCompile(`^[A-Z]{3}$`)

// readJSONBody reads the bounded raw request body once and decodes it into
// dst, rejecting unknown fields (api/openapi/b2b-v1.yaml's
...
// positive integer decimal string, matching api/openapi/components/common.yaml's
// MoneyMinor pattern (`^[0-9]+$`) plus the "positive" requirement every
// owner-service create path already enforces
// (internal/payin/grpcserver.parseUserAndAmount and its payout twin) — a
...
// validateCurrency enforces api/openapi/components/common.yaml's Currency
