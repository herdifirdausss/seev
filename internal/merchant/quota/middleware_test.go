...
func TestRequireQuota_OverBurst_429WithRetryAfter(t *testing.T) {
	...
	handler.ServeHTTP(rec1, newTestRequest(t, tenantID))
	require.Equal(t, http.StatusCreated, rec1.Code)

	rec2 := httptest.NewRecorder()
	handlerCalled := false
	handler2 := RequireQuota(enforcer, "transfers", true)(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		handlerCalled = true
	}))
	handler2.ServeHTTP(rec2, newTestRequest(t, tenantID))
