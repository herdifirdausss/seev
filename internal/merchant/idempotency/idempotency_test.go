...
func TestCanonicalRequestHash_DifferentOperationIDs_ProduceDifferentHashes(t *testing.T) {
	body := []byte(`{"amount":100}`)
	h1 := CanonicalRequestHash("op.transfer", body)
	h2 := CanonicalRequestHash("op.refund", body)
	assert.NotEqual(t, h1, h2)
}

func TestDownstreamKey_StableAcrossCalls(t *testing.T) {
	tenantID := uuid.New()
	k1 := DownstreamKey(tenantID, "op.transfer", "key-1")
	k2 := DownstreamKey(tenantID, "op.transfer", "key-1")
