...
// local structural interface (mirrors internal/notify's own Broker) so
...
// goroutine — matches internal/notify.Module.Start's own shape exactly
...
// internal/notify's own handleDelivery: a malformed payload never succeeds
// differently on redelivery, and pkg/messaging's own shouldRequeue already
...
func (c *Consumer) handleDelivery(ctx context.Context, d amqp.Delivery) error {
	...
	whEvent := model.WebhookEvent{
		ID: generalutil.NewV7(), PublicID: publicID, TenantID: tenantID,
	...
	for _, endpoint := range endpoints {
		...
		delivery := model.WebhookDelivery{
			ID: generalutil.NewV7(), PublicID: "whd_" + uuid.NewString()[:16], TenantID: tenantID,
