...
// encoding convention internal/merchant/auth's API keys already use.
...
// internal/merchant/auth.KeyService.CreateKey's own one-time-display
...
// reads so callers never need to import internal/merchant/repository
// directly (module-boundary convention already established by every
// other internal/merchant subpackage this session).
