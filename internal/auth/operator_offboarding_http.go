...
// role helpers exactly (internal/ledger/transport/http.go) — the "admin"
...
func (m *Module) AdminProposeOperatorOffboardingHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		proposal, err := m.ProposeOperatorOffboarding(r.Context(), requestedBy.String(), targetUserID, req.Reason)
		if err != nil {
...
func (m *Module) AdminApproveOperatorOffboardingHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		decided, err := m.ApproveOperatorOffboarding(r.Context(), id, approvedBy.String())
		if err != nil {
			writeOperatorOffboardingError(w, err)
			return
		}
...
func (m *Module) AdminRejectOperatorOffboardingHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		decided, err := m.RejectOperatorOffboarding(r.Context(), id, approvedBy.String())
		if err != nil {
			writeOperatorOffboardingError(w, err)
			return
		}
...
func (m *Module) AdminListOperatorOffboardingHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		requests, err := m.ListOperatorOffboarding(r.Context(), r.URL.Query().Get("status"), 100)
		if err != nil {
			response.InternalServerError(w, err)
			return
		}
