...
func TestIntegration_MultipleJobs(t *testing.T) {
	...
	var firstJobRunCount, secondJobRunCount atomic.Int32

	firstJob := testImmediateJob(t, "job1", func(ctx context.Context) error {
		firstJobRunCount.Add(1)
		return nil
	})

	secondJob := testImmediateJob(t, "job2", func(ctx context.Context) error {
		secondJobRunCount.Add(1)
		return nil
	})
	...
	firstJobRuns := firstJobRunCount.Load()
	secondJobRuns := secondJobRunCount.Load()

	if firstJobRuns == 0 || secondJobRuns == 0 {
		t.Errorf("jobs not executed: job1=%d, job2=%d", firstJobRuns, secondJobRuns)
	}
}
...
func BenchmarkMemoryLockTryLock(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		_, _ = lock.TryLock(ctx, key, 1*time.Second)
	}
}
