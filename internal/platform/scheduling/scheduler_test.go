...
func TestBrutal_SchedulerRaceConditions(t *testing.T) {
	...
	t.Run("lock prevents double exec", func(t *testing.T) {
		...
		firstScheduler := NewScheduler(lock, nil, WithDefaultTimeout(5*time.Second))
		defer firstScheduler.Stop()

		secondScheduler := NewScheduler(lock, nil, WithDefaultTimeout(5*time.Second))
		defer secondScheduler.Stop()

		firstJobFunction := func(ctx context.Context) error {
			counter.Add(1)
			time.Sleep(100 * time.Millisecond)
			return nil
		}

		secondJobFunction := func(ctx context.Context) error {
			counter.Add(1)
			time.Sleep(100 * time.Millisecond)
			return nil
		}

		firstJob := testImmediateJob(t, "immediate-job-1", firstJobFunction)
		secondJob := testImmediateJob(t, "immediate-job-2", secondJobFunction)
		firstScheduler.Add(firstJob)
		secondScheduler.Add(secondJob)
