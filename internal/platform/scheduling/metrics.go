...
// jobSkipsTotal is docs/roadmap/archive/45 K6's scheduler_job_skips_total{job}
...
var jobSkipsTotal = promauto.NewCounterVec(prometheus.CounterOpts{
	...
	Help:      "Cron job ticks skipped because the distributed lock could not be acquired (docs/roadmap/archive/45 K6) — e.g. Redis unavailable.",
}, []string{"job"})

// PrometheusMetrics implements Metrics, backing JobSkip with the
// scheduler_job_skips_total counter (docs/roadmap/archive/45 Task T3/K4 — the
