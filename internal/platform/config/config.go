...
type Config struct {
	...
	// just production, and is enforced directly by services/ledger/cmd/ledger's
	...
	// TLSCertDir is where tools/certgen (docs/roadmap/archive/49 K3) writes ca.pem plus
	...
	// internal/platform/security/tls. Never a per-file env var: one directory convention keeps
...
// false, so services/gateway/cmd/gateway registers zero vendors and every /webhooks/{vendor}
...
// services/gateway/cmd/gateway/main.go, never a change to services/payin.
type VendorConfig struct {
	...
	// SecondaryMockVendorEnabled/Secret register a SECOND mock vendor (docs/roadmap/archive/40
	...
	// feature existed.
	SecondaryMockVendorEnabled bool
	SecondaryMockVendorSecret  string
...
// contracts/vendorgw.HealthTracker) shared by payin-service and
// payout-service.
...
type TracingConfig struct {
	...
	// existing span-creation code (services/ledger/internal/ledger/handle's
...
// services/ledger itself (the module must not depend on the composition
// root's config type — see services/ledger.WorkerConfig for the same
// pattern).
...
// internal/platform/security/crypto key set — one KEK ring and one lookup key for every service,
...
// services comes from internal/platform/security/crypto.AAD's Service/Table/Column binding, not
...
type AppConfig struct {
	...
	// per-IP(+path) rate limiter (services/auth/cmd/auth/router.go,
	// services/gateway/internal/transport/http/router.go). Defaults (10 per minute) match this
...
type WorkerConfig struct {
	Enabled bool
	// FinancialProductsEnabled is deliberately opt-in: migrations and maker/checker controls
...
func loadFromEnvMode(getenv func(string) string, requireRabbitMQ bool) (*Config, error) {
	...
	cfg := &Config{
		...
		Worker: WorkerConfig{
			...
			FinancialProductsEnabled: parseBool(getenv("C5_FINANCIAL_PRODUCTS_ENABLED"), false),
		...
		Vendor: VendorConfig{
			...
			SecondaryMockVendorEnabled: parseBool(getenv("MOCKVENDOR2_ENABLED"), false),
			SecondaryMockVendorSecret:  getenv("MOCKVENDOR2_SECRET"),
...
// BACKUP_PASSWORD_FILE convention operations/agents/backup/config.go already
...
func validate(cfg *Config, requireRabbitMQ bool, errs *[]string) error {
	...
	if cfg.Vendor.SecondaryMockVendorEnabled && cfg.Vendor.SecondaryMockVendorSecret == "" {
