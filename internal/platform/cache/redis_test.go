...
func TestCache_GetOrSet_CacheMiss_FetchSuccess(t *testing.T) {
	...
	fetchCalled := false
	var secondDestination map[string]any
	err = c.GetOrSet(ctx, "item:1", time.Minute, &secondDestination, func() (any, error) {
		fetchCalled = true
		return nil, nil
	})
	require.NoError(t, err)
...
func TestCache_GetOrSet_StoreFailsButReturnsValue(t *testing.T) {
	...
	secondRedis, err := miniredis.Run()
	require.NoError(t, err)

	secondClient := redis.NewClient(&redis.Options{Addr: secondRedis.Addr()})
	secondCache := NewFromClient(secondClient)

	err = secondCache.GetOrSet(ctx, "key", time.Minute, &dest, func() (any, error) {
		secondRedis.Close() // Kill server so subsequent Set fails → triggers slog.Warn path
