...
// internal/platform/transport/httpserver/server_test.go
...
func TestStartMulti_GracefulShutdown(t *testing.T) {
	...
	publicServer := New(cfg, okHandler)
	internalServer := New(cfg, okHandler)
	...
	go func() {
		...
		}, []os.Signal{syscall.SIGUSR2}, publicServer, internalServer)
	}()
