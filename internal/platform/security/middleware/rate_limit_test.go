...
func TestRateLimitByIP_StripsEphemeralPort(t *testing.T) {
	...
	firstRequest.RemoteAddr = "203.0.113.5:1234"

	secondRequest := httptest.NewRequest(http.MethodGet, "/", nil)
	secondRequest.RemoteAddr = "203.0.113.5:59999"

	firstRateLimitKey := RateLimitByIP(firstRequest)
	secondRateLimitKey := RateLimitByIP(secondRequest)

	assert.Equal(t, firstRateLimitKey, secondRateLimitKey, "same client IP with different ephemeral ports must share one rate-limit bucket")
	assert.Equal(t, "rl:ip:203.0.113.5", firstRateLimitKey)
}
...
func TestRateLimitByIPAndPath_StripsEphemeralPort(t *testing.T) {
	...
	assert.Equal(t, RateLimitByIPAndPath(firstRequest), RateLimitByIPAndPath(secondRequest))
	assert.Equal(t, "rl:203.0.113.5:/login", RateLimitByIPAndPath(firstRequest))
}
