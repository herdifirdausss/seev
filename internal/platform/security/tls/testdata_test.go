...
// tools/certgen (which this package must not import, and which exercises
// its own issuance logic in its own tests) so internal/platform/security/tls's tests only ever
...
// the certificate's sole URI SAN — identical shape to what tools/certgen
// produces, so internal/platform/security/tls's tests exercise the real consumer contract.
