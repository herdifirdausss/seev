...
func TestCertExpirySeconds_SetOnReload(t *testing.T) {
	...
	reissuedCertSource, err := newCertSource(filepath.Join(dir, "ledger.pem"), filepath.Join(dir, "ledger-key.pem"), filepath.Join(dir, "ca.pem"), nil, fastPoll)
	if err != nil {
		t.Fatalf("NewCertSource (post-reissue): %v", err)
	}
	defer reissuedCertSource.Stop()
