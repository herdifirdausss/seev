...
// tools/certgen rewrites it on disk (docs/roadmap/archive/49 K2/K9 — rotation is
...
// (internal/platform/transport/grpc AND every internal HTTP server/client), so a rotation is
...
// LoadFromDir applies this repo's one directory convention (tools/certgen's
...
// tools/certgen rewrites cert+key+ca together, but a stat-based check keeps
...
// identityOf extracts the single SPIFFE-style URI SAN a tools/certgen leaf
