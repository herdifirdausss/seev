...
func TestServerConfig_HotRotatesCAWithoutRebuildingConfig(t *testing.T) {
	dir := t.TempDir()
	initialCA := newTestCA(t)
	initialCA.writeTo(t, dir)
	initialCA.issue(t, dir, "ledger", IdentityLedger, time.Hour)
	initialCA.issue(t, dir, "gateway", IdentityGateway, time.Hour)
	...
	rotatedCA.writeTo(t, dir)
	rotatedCA.issue(t, dir, "ledger", IdentityLedger, time.Hour)
	rotatedCA.issue(t, dir, "gateway", IdentityGateway, time.Hour)
