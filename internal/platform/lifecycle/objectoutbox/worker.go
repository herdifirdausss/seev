...
// passes a compile-time constant), matching internal/platform/lifecycle/retention/worker's
...
// internal/platform/database.DatabaseSQL, matching this repo's own narrow-interface
// convention (internal/platform/lifecycle/retention/worker.dbQuerier, services/payout/internal/worker's
...
// tables" applies here the same as internal/platform/lifecycle/retention/worker.
type Worker struct {
	owner       string
	db          dbTxer
	store       Store
...
// interval, matching services/ledger/internal/worker.OutboxRelay's Start/Stop
