...
// matching tools/doccheck's own "report everything, then exit 1 once" style.
...
func validateAgainstMigrations(root string, tablesInPolicy map[string]bool) []string {
	var errs []string
	ownerDirs, err := migrationOwnerDirs(root)
	if err != nil {
	...
	for _, ownerDir := range ownerDirs {
		owner := ownerDir.owner
		files, err := filepath.Glob(filepath.Join(ownerDir.path, "*.up.sql"))
		if err != nil {
...
type migrationOwnerDir struct {
	owner string
	path  string
}

// migrationOwnerDirs accepts both the canonical repository root
// (services/<physical-service>/migrations) and the older aggregate migrations
// root (migrations/<owner>). This keeps the policy validator useful to local
// tooling while making the service-owned layout the primary path.
func migrationOwnerDirs(root string) ([]migrationOwnerDir, error) {
	servicesRoot := root
	...
	var owners []migrationOwnerDir
	for _, entry := range entries {
		if !entry.IsDir() {
			continue
		}
		physical := entry.Name()
		migrationPath := filepath.Join(servicesRoot, physical, "migrations")
		if servicesRoot == root {
			migrationPath = filepath.Join(root, physical)
		}
		if info, statErr := os.Stat(migrationPath); statErr != nil || !info.IsDir() {
			continue
		}
		owner := physical
		...
		owners = append(owners, migrationOwnerDir{owner: owner, path: migrationPath})
	}
	if len(owners) == 0 {
		return nil, fmt.Errorf("no owner migration directories found under %s", root)
	}
