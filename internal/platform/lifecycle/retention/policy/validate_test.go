...
// validates it against the actual, committed service-owned migrations/ tree — this is
...
func realRepoPolicy(t *testing.T) *Policy {
	t.Helper()
	p, err := LoadPolicy(filepath.Join(repoRoot(), "config", "data-retention.yaml"))
	if err != nil {
...
func TestValidate_AllOwnerMigrationDirectoriesCovered(t *testing.T) {
	entries, err := os.ReadDir(filepath.Join(realMigrationsRoot(), "services"))
	if err != nil {
		t.Fatal(err)
	}
	var owners []string
	for _, e := range entries {
		if e.IsDir() {
			migrationDir := filepath.Join(realMigrationsRoot(), "services", e.Name(), "migrations")
			if _, statErr := os.Stat(migrationDir); statErr != nil {
				continue
			}
			owner := e.Name()
			if owner == "vendor-service" {
				owner = "vendor"
			}
	...
	for _, w := range want {
		...
		if !found {
			t.Errorf("expected a service-owned migrations/%s directory", w)
		}
	}
}
...
func TestRenderMarkdown_MatchesCommittedDoc(t *testing.T) {
	...
	committed, err := os.ReadFile(filepath.Join(repoRoot(), "docs", "data", "retention.md"))
	if err != nil {
		t.Fatal(err)
	}
