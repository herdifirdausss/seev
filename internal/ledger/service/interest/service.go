...
type Service struct {
	db       DatabaseSQL
	repo     repository.C5InterestRepository
...
func New(db DatabaseSQL, repo repository.C5InterestRepository, snapshot BalanceReader, poster Poster, logger *slog.Logger, loc *time.Location) *Service {
	if logger == nil {
		logger = slog.Default()
	}
	if loc == nil {
