...
// through to a processor on the PUBLIC router (docs/plan/10 Task T3):
...
// persisted to ledger_transactions.external_ref (docs/plan/16 Task T2, K5)
// — rejected outright rather than silently truncated (same principle as
// amount integrality, docs/plan/10 Task T4: a caller relying on the full
...
// set (docs/plan/38 Task T4), fee resolution is SKIPPED here entirely —
...
func (h *handler) buildMetadata(ctx context.Context, userID uuid.UUID, req postTransactionRequest, amount decimal.Decimal) (map[string]any, error) {
	...
	// WithRequestID attached (docs/plan/36 Task T5), so this is the
	// authoritative end-to-end trace anchor for the posted transaction.
	if id := middleware.RequestIDFromCtx(ctx); id != "" {
		out["request_id"] = id
	}
	// currency is best-effort (docs/plan/18 Task T2): if it can't be
