...
// The eight metric names below are docs/plan/50 K13's exact,
...
var (
	backupLastSuccessTimestamp = promauto.NewGaugeVec(prometheus.GaugeOpts{
		...
		Help: "Unix timestamp of the last successful backup, by type (docs/plan/50 K13).",
	}, []string{"type"})

	backupDuration = promauto.NewGaugeVec(prometheus.GaugeOpts{
		Name: "seev_backup_duration_seconds",
		Help: "Duration of the most recent backup attempt, by type and result (docs/plan/50 K13).",
	}, []string{"type", "result"})

	backupSizeBytes = promauto.NewGaugeVec(prometheus.GaugeOpts{
		Name: "seev_backup_size_bytes",
		Help: "Size of the most recent successful backup, by type (docs/plan/50 K13).",
	}, []string{"type"})

	backupRepositoryCheckTotal = promauto.NewCounterVec(prometheus.CounterOpts{
		Name: "seev_backup_repository_check_total",
		Help: "pgBackRest repository check attempts, by result (docs/plan/50 K13).",
	}, []string{"result"})

	backupWALArchiveAge = promauto.NewGauge(prometheus.GaugeOpts{
		Name: "seev_backup_wal_archive_age_seconds",
		Help: "Seconds since PostgreSQL last successfully archived a WAL segment (docs/plan/50 K13) — crossing the RPO budget means archiving is stuck.",
	})

	backupOldestRestorePoint = promauto.NewGauge(prometheus.GaugeOpts{
		Name: "seev_backup_oldest_restore_point_timestamp_seconds",
		Help: "Unix timestamp of the earliest point-in-time this repository can currently restore to (docs/plan/50 K13).",
	})
	...
	drDrillRPO = promauto.NewGaugeVec(prometheus.GaugeOpts{
		...
		Help: "Recovery point objective observed by the most recent DR drill, by mode and result (docs/plan/50 K12/K13).",
	}, []string{"mode", "result"})

	drDrillRTO = promauto.NewGaugeVec(prometheus.GaugeOpts{
		Name: "seev_dr_drill_rto_seconds",
		Help: "Recovery time objective observed by the most recent DR drill, by mode and result (docs/plan/50 K12/K13).",
	}, []string{"mode", "result"})
)

// RecordDrillResult publishes the two DR-drill metrics — called by the
// docs/plan/50 T6 game-day drill script once it has measured both
