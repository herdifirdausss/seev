...
var validPayoutGateways = map[string]bool{"bca": true, "gopay": true, "platform": true}
...
func (m *Module) validateRoutingPayload(p routingRulePayload) (model.RoutingRule, string) {
	if p.Flow == "" {
		p.Flow = "payout"
	}
	if p.Flow != "payout" {
		return model.RoutingRule{}, "flow must be payout"
	}
	if p.Priority < 0 {
		return model.RoutingRule{}, "priority must be non-negative"
	}
	if _, ok := m.registry.Payout(p.Vendor); !ok {
...
func (m *Module) createRoutingRuleHandler(w http.ResponseWriter, r *http.Request) {
	...
	rule, msg := m.validateRoutingPayload(p)
	if msg != "" {
...
func (m *Module) putVendorGatewayHandler(w http.ResponseWriter, r *http.Request) {
	...
	if _, ok := m.registry.Payout(vendor); !ok {
	...
	if !validPayoutGateways[p.Gateway] {
		response.BadRequest(w, "gateway is not allowed")
		return
	}
	mapping := model.VendorGateway{Vendor: vendor, Gateway: p.Gateway}
