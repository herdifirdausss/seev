...
Covers seeding the local dev-mode Vault (docs/plan/49 K7) that `internal/config`'s `vaultGetenv` seam can overlay on top of plain env vars when `VAULT_ADDR`/`VAULT_TOKEN` are set. Dev-mode Vault is **in-memory** — every restart wipes it, and this seed script exists precisely because "reseed on every boot" is the normal, expected operating mode here, not an edge case.
...
- `JWT_ISSUER` — not a secret (docs/plan/49 TM-07 made it mandatory, but it's a plain identifier, not sensitive). `vaultGetenv` falls through to the underlying env var for any key Vault doesn't have — confirmed by `TestVaultGetenv_FallsThroughToEnvForKeysVaultDoesNotHave` — so a service booting with `VAULT_ADDR`/`VAULT_TOKEN` set still gets `JWT_ISSUER` from its regular env (`.env.example`'s `JWT_ISSUER=seev` default, or whatever Compose/`scripts/lib.sh` set) without needing a Vault entry at all.
...
Dev-mode Vault talks plain HTTP on the same Compose network every other service is on (docs/plan/49 K7/TM-10, accepted-risk) — secrets fetched from Vault cross that hop without TLS unless a future mTLS sweep explicitly adds Vault as a known identity (it is not one of K3/K4's identities today). A TLS listener for Vault itself is an explicit follow-up outside doc 49's scope, not something this seed script or runbook can fix.
...
- [docs/plan/49-a6-internal-security.md](../plan/49-a6-internal-security.md) K7 — the dev-mode-vs-production-Vault decision and its stated trade-offs.
