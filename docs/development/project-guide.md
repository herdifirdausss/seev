...
> **Status: Current. Audience: contributors.** These are implementation rules,
> not an introduction to the product. New readers should begin with the
> [beginner guide](../learn/beginner-guide.md) and [onboarding](onboarding.md).
...
changes may use `make docs-check` plus `git diff --check`; CI runs the same
documentation check without starting the heavy runtime jobs.
...
See [Operations](../operations/README.md) for what each script proves and why,
what every chaos scenario specifically tests, and the same depth of detail
for `docker-compose.yml`, the Makefile, CI, and the observability stack.
