...
| 26 | [26-phase6a-foundations.md](26-phase6a-foundations.md) | **Master reference split microservices (baca dulu sebelum 27–35)** + foundations: lunasi hutang `pkg/→internal/config`, toolchain buf/gRPC, `pkg/grpcx`+`pkg/ledgererr`, restrukturisasi migrasi per-service | ✅ done |
| 27 | [27-phase6b-ledger-service.md](27-phase6b-ledger-service.md) | Ekstraksi ledger-service (+policy): `ledger.proto`, gRPC server + mapping error, `pkg/ledgerclient`, re-type interface konsumen, cutover `seev_ledger`, boundary v2 | ✅ done |
| 28 | [28-phase6c-auth-service.md](28-phase6c-auth-service.md) | Ekstraksi auth-service (PUBLIC `:8082`): DB `seev_auth`, ProvisionUser via gRPC, gateway melepas route auth | ✅ done |
| 29 | [29-phase6d-payin-service-routing.md](29-phase6d-payin-service-routing.md) | Ekstraksi payin-service (INTERNAL) + **routing topup DB-driven** (`payin_routing_rules`), webhook edge tetap di gateway (forward raw bytes via gRPC) | ✅ done |
| 30 | [30-phase6e-payout-service-routing.md](30-phase6e-payout-service-routing.md) | Ekstraksi payout-service (INTERNAL) + **routing payout DB-driven**, resume job ikut, chaos crash-mid-flight lintas service WAJIB | ✅ done |
| 31 | [31-phase6f-fraud-service.md](31-phase6f-fraud-service.md) | Ekstraksi fraud-service: modul `internal/fraud`, `screening_events`→`seev_fraud`, hook gRPC fail-open 500ms di seam PrePostHook, velocity via consumer event | ✅ done |
| 32 | [32-phase6g-gateway-service.md](32-phase6g-gateway-service.md) | Formalisasi gateway-service: rename `cmd/server`→`cmd/gateway`, notify DB→`seev_gateway`, boundary final, compose profile `app` enam service | ✅ done |
| 33 | [33-phase6h-fee-rules.md](33-phase6h-fee-rules.md) | **Fee DB-driven per-user-per-route**: tabel `fee_rules`, feepolicy DB-backed dengan spesifisitas user+route, admin CRUD, hapus env `FEE_*` | ✅ done |
...
| 42 | [42-long-term-roadmap.md](42-long-term-roadmap.md) | Roadmap jangka panjang pasca-MVP: 20 track dalam 3 horizon (fondasi ops / kondisional-terukur / aspirasional) dengan trigger aktivasi — dokumen eksekusi 43+ lahir dari sini | referensi |
...
| 44 | [44-a2-ci-pipeline.md](44-a2-ci-pipeline.md) | **Track A2 (bagian CI) — full-stack gate**: smoke container pada PR, business E2E + seluruh chaos pada scheduled full-stack mingguan/manual, build image tanpa push, artefak diagnostik, pinning action, dan batas resource runner | ✅ done |
...
| 48 | [48-a10-product-assurance.md](48-a10-product-assurance.md) | **Track A10 — product assurance & emergency intake control**: verifier durable korelasi payin–payout–ledger, finding/alert lifecycle, operator API/CLI, serta pause intake maker/checker tanpa auto-remediation | ✅ done (T0–T6 + acceptance/final gate hijau; 2026-07-20) |
| 49 | [49-a6-internal-security.md](49-a6-internal-security.md) | **Track A6 — keamanan internal**: threat model STRIDE + register temuan, mTLS SPIFFE-ish (mini-CA Go `cmd/certgen`, URI SAN, hot-reload rotation) untuk SEMUA hop gRPC+HTTP internal + matriks allowlist per-hop, token internal fail-closed, Vault dev-mode (profile `secrets`) dengan fallback env, review pentest-style ber-bukti + fix temuan, rotation drill | ✅ done (T1–T6 + GATE 3 terisolasi hijau, 2026-07-22) |
