...
|---|---|---|
| Gateway `services/gateway/internal/notification` | notification history, event inbox, kind policy, template lookup, planning, user settings/preferences/devices, durable email/push/digest delivery, channel controls | balances, payment decisions, Auth data, provider business state |
| Auth `services/auth` | user identity, account status, verified email state, internal contact contract | notification history or delivery state |
