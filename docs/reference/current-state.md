...
| C5 | Savings products/rates, daily accrual and period-close foundations, durable schedule occurrences/policies, and top-up fee paths are implemented in Ledger. Core unit tests (interest math, service, schedule policy/durable), integration tests (interest period-close, durable schedule happy path + idempotent replay + daily-skip policy) against real Postgres, and E2E journey scripts (interest-period-e2e, schedule-policy-e2e, topup-fee-e2e) are present. Chaos/load/Gateway-BFF exposure/operations gates remain deferred. Runtime evidence recorded 2026-08-05; plan archived — see [c5-final-acceptance.md](../evidence/c5-final-acceptance.md). |
| C6 | The `account_balances_v2` migration control plane, backfill, dual-write/shadow-read, target-primary cutover, repair, and rollback machinery are implemented and verified. Core invariants covered by unit + integration tests; E2E drill scripted (`scripts/migration-balance-v2-e2e.sh`). Chaos/load/PITR gates deferred (tracked follow-ups in plan 62 §42). |
