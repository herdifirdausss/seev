...
| TM-06 | `pkg/middleware/cors.go:25` default `AllowedOrigins: []string{"*"}` — permisif untuk SEMUA origin (mitigasi parsial: `AllowCredentials:false`). Dipakai di service mana pun yang memanggil `DefaultCORSConfig()` tanpa override. | **Medium** | T5 (verifikasi pemakaian nyata) → T6 (fix ke allowlist eksplisit atau kosongkan untuk API-only) | open — **dikonfirmasi live T5**: preflight `OPTIONS` dari origin sembarang (`https://evil.attacker.example`) mendapat `Access-Control-Allow-Origin: *`. Risiko dibatasi `AllowCredentials:false` (tidak ada leak cookie cross-origin), tapi tetap mengizinkan skrip pihak ketiga memanggil API secara terprogram jika token sudah didapat lewat jalur lain. |
| TM-07 | `pkg/middleware/auth.go:94` — validasi issuer JWT di-skip total saat `expectedIssuer==""`; `JWT_ISSUER` tidak wajib diisi di config manapun (hanya warning produksi). Token dari konfigurasi lain (atau dites lupa di-set) tetap diterima. | **Medium** | T5 (konfirmasi dampak nyata) → T6 (wajibkan issuer di semua service) | open — **dikonfirmasi live T5**: token bertanda tangan valid dengan klaim `iss` sembarang (`https://totally-not-seev.attacker.example`) DITERIMA (200) karena `JWT_ISSUER` kosong di seluruh compose/lib.sh hari ini. Risiko utama: tidak ada perlindungan terhadap secret yang sama tidak sengaja dipakai lintas environment. |
| TM-08 | Webhook HMAC (`internal/vendorgw/mockvendor/mockvendor.go`) tidak mengikat timestamp ke dalam signature — replay dibatasi murni oleh freshness bisnis `OccurredAt` + dedup `VendorEventID`, bukan kriptografis. | **Low** | T5 (nilai risiko nyata: apakah `VendorEventID` cukup unpredictable) → T6 (perbaiki atau accepted-risk tertulis) | open — **dikonfirmasi live T5**: webhook dengan `occurred_at` 10 tahun lalu DITERIMA (200) dan diproses. Risiko dibatasi ganda: (a) signature tetap wajib valid (secret vendor tidak bocor dari test ini), (b) dedup `VendorEventID` mencegah REPLAY event yang sama diproses dua kali (dibuktikan live T5 — request kedua dengan `event_id` sama tidak menggandakan saldo). |
