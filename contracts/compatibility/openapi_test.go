...
func TestExceptionalHTTPRepresentationsAreExplicit(t *testing.T) {
	common, err := os.ReadFile(filepath.Join("..", "http", "components", "common.yaml"))
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, name := range []string{"public-v1.yaml", "admin-v1.yaml"} {
		body, err := os.ReadFile(filepath.Join("..", "http", name))
		if err != nil {
			t.Fatal(err)
		}
		text := string(body)
	...
	public, err := os.ReadFile(filepath.Join("..", "http", "public-v1.yaml"))
	if err != nil {
		t.Fatal(err)
	}
