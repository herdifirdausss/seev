...
func TestPayoutProvider_Async_ReturnsPending_ThenCompletePendingSettles(t *testing.T) {
	...
	settledPayout, err := p.Query(context.Background(), "key-2")
	require.NoError(t, err)
	assert.Equal(t, vendorgw.PayoutSettled, settledPayout.Status)
}
...
func TestPayoutProvider_Timeout_ReturnsErrorEveryTime(t *testing.T) {
	...
	_, timeoutErr := p.Submit(context.Background(), "key-4", decimal.NewFromInt(10_000), "IDR", destJSON(t, ModeTimeout))
