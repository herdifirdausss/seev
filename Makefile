...
## supply-chain-e2e: Validate source controls and a protected-release bundle end to end
supply-chain-e2e:
	./scripts/ci/supply-chain-e2e.sh
...
## analytics-metrics-exporter: Serve bounded Prometheus metrics on 127.0.0.1:9308
analytics-metrics-exporter:
	go run ./analytics/reconciliation/cmd/metrics-exporter
...
k0-inventory:
	@./scripts/deployment-inventory-generate.sh

## k0-inventory-check: Validate K0 machine-readable contracts and evidence safety
k0-inventory-check:
	@./scripts/deployment-inventory-check.sh

## k0-resource-sample: Collect a bounded Docker resource sample; never starts containers
k0-resource-sample:
	@./scripts/deployment-resource-sample.sh

## k0-network-probe: Probe only declared local endpoints and broker topology
k0-network-probe:
	@./scripts/deployment-network-probe.sh

## helm-lint: Validate the Plan 63 application chart (requires Helm 3)
helm-lint:
	@command -v helm >/dev/null 2>&1 || { echo "helm is required" >&2; exit 2; }
	helm lint deploy/helm/seev
	helm lint deploy/helm/seev -f deploy/helm/seev/values-local.yaml
...
## platform-e2e: Render platform identity contracts into application workloads
platform-e2e:
	@./scripts/platform-e2e.sh
...
## k8s-e2e: Bootstrap local Kubernetes, run edge smoke, and execute the business journey
k8s-e2e:
	@$(MAKE) --no-print-directory k8s-bootstrap
	@$(MAKE) --no-print-directory k8s-smoke
...
## k8s-verify-callback: Check callback source-IP and route controls
k8s-verify-callback:
	@./deploy/kubernetes/scripts/verify-callback-allowlist.sh

## k8s-verify-egress: Check direct egress denial and Squid destination policy
k8s-verify-egress:
...
## dr-e2e: Disposable latest and PITR restore journeys with post-restore application smoke
dr-e2e:
	@./scripts/dr-e2e.sh
...
## migration-contract-check: Build and vet the two C6 migration packages (static gate)
migration-contract-check:
	go build ./internal/platform/migration/... ./services/ledger/internal/migration/...
	go vet ./internal/platform/migration/... ./services/ledger/internal/migration/...

## migration-state-test: Fast unit tests for the migration state machine and sampling (no DB)
migration-state-test:
	go test ./internal/platform/migration/... ./services/ledger/internal/migration/balancev2/...

## migration-balance-v2-e2e: Full lifecycle drill for the C6 balance-projection migration (requires docker-compose stack)
migration-balance-v2-e2e:
	./scripts/migration-balance-v2-e2e.sh

## migration-verify: Aggregates the C6 static, unit, and E2E gates
migration-verify:
	@set -eu; \
	$(MAKE) --no-print-directory migration-contract-check; \
	$(MAKE) --no-print-directory migration-state-test; \
	$(MAKE) --no-print-directory migration-balance-v2-e2e
