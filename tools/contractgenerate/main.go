...
// bundles from contracts/http/*-v1.yaml. It resolves only checked-in relative
...
func main() {
	outDir := flag.String("out", "contracts/http/dist", "bundle output directory")
	flag.Parse()
...
func generate(outDir string) error {
	...
	for _, name := range sourceNames {
		path := filepath.Join("contracts", "http", name)
