...
func main() {
	...
	baselineDir := flag.String("baseline-dir", filepath.Join("contracts", "compatibility", "baseline", "openapi"), "directory containing the merge-base bundles")
	currentDir := flag.String("current-dir", filepath.Join("contracts", "http", "dist"), "directory containing the current bundles")
	approvedPath := flag.String("approved-breakings", filepath.Join("contracts", "compatibility", "approved-breaking.yaml"), "explicitly reviewed intentional cutovers")
...
func readBaseline(directory, ref, name string) ([]byte, error) {
	...
	cmd := exec.Command("git", "show", ref+":contracts/http/dist/"+name)
	body, err := cmd.Output()
	if err != nil {
...
// $ref (tools/contractgenerate leaves the resolved object nested under a
