...
// per-service leaf certificates that internal/platform/security/tls loads for mutual TLS between
...
// identity internal/platform/security/tls expects — the closed set from internal/platform/security/tls.Identity*
// constants (docs/roadmap/archive/49 K3/K4). Adding a service here is the ONE place
// certgen needs to change; internal/platform/security/tls's allowlists are a separate, per-
...
// the moment internal/platform/security/tls's poll-reload picks up the new ca.pem (docs/roadmap/archive/49
...
func issueLeaf(out, service, identity string, ca *x509.Certificate, caKey *ecdsa.PrivateKey) error {
	...
	// (internal/platform/security/tls.CertSource) — running as a non-root, non-matching uid.
