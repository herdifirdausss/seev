...
`loaddataset` is a read-only observer (same posture as `tools/loadprobe`) that
...
- **Not `tools/loadseed`** — that tool emits deterministic *synthetic* JSONL
