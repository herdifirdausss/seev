...
func run() error {
	...
	// The migration runner receives the service roots, not a flat repository
	// migration bucket. This keeps schema ownership next to the service that
	// deploys it.
	root := envOr("MIGRATIONS_ROOT", "/app/services")
	...
	for _, service := range services {
		...
		source := "file://" + filepath.Join(root, migrationDirectory(service), "migrations")
...
func migrationDirectory(service string) string {
	if service == "vendor" {
