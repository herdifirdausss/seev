-- payout_requests.fee_amount (added nullable, migrations/payout/000004_quoted_fee.up.sql)
-- had no non-negativity CHECK, unlike its source fee_quotes.fee_amount
-- (CHECK (fee_amount >= 0 AND fee_amount < amount), migrations/ledger/000021_fee_quotes.up.sql)
-- and unlike internal/payout/orchestrate.go's own use of FeeAmount.IsPositive()
